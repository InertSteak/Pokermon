return {
	descriptions = {
		Back = {
			b_poke_luminousdeck = {
				name = "发亮牌组",
				text = {
					"所有小丑牌附有",
					"随机的{C:pink}属性{}贴纸",
					"及{C:attention}+1{}{C:pink}能量{}"
				}
			},
			b_poke_obituarydeck = {
				name = "讣闻牌组",
				text = {
					"所有卡牌有{C:dark_edition}#1#{}"
				}
			},
			b_poke_pokemondeck = {
				name = "训练家牌组",
				text = {
					"开局时获得",
					"{C:tarot,T:v_poke_goodrod}#1#{}优惠券及",
					"{C:tarot,T:c_poke_pokeball}#2#{}卡"
				}
			}
		},
		Blind = {
			bl_poke_cgoose = {
				name = "夏翠丝密室",
				text = {
					"每次出牌时",
					"对其中随机一个属性减益"
				}
			}
		},
		Edition = {
			e_poke_shiny = {
				label = "发光",
				name = "发光",
				text = {
					"店内的扩充包字段{C:attention}+1{}"
				}
			}
		},
		Energy = {
			c_poke_colorless_energy = {
				name = "无色能量",
				text = {
					"增加最左边的小丑牌的",
					"{C:attention}加分{}及{C:money}金钱{}数值",
					"对非{C:attention}一般{}属性小丑牌的增强效果减半",
					"{C:inactive}(每张小丑牌可增强{C:attention}#1#{}{C:inactive}次)"
				}
			},
			c_poke_darkness_energy = {
				name = "恶能量",
				text = {
					"增加最左边的{C:attention}恶{}属性小丑牌的",
					"{C:attention}加分{}及{C:money}金钱{}数值",
					"{C:inactive}(每张小丑牌可增强{C:attention}#1#{}{C:inactive}次)"
				}
			},
			c_poke_dragon_energy = {
				name = "龙能量",
				text = {
					"增加最左边的{C:attention}龙{}属性小丑牌的",
					"{C:attention}加分{}及{C:money}金钱{}数值",
					"{C:inactive}(每张小丑牌可增强{C:attention}#1#{}{C:inactive}次)"
				}
			},
			c_poke_earth_energy = {
				name = "地能量",
				text = {
					"增加最左边的{C:attention}地面{}属性小丑牌的",
					"{C:attention}加分{}及{C:money}金钱{}数值",
					"{C:inactive}(每张小丑牌可增强{C:attention}#1#{}{C:inactive}次)"
				}
			},
			c_poke_emergy = {
				name = "M能量",
				text = {
					"每张{C:attention}开心小丑{}牌或{C:legendary}M 小丑牌{}会",
					"产生{C:attention}一张{}随机带有{C:dark_edition}负片{}的{C:green}能量牌{}"
				}
			},
			c_poke_fairy_energy = {
				name = "妖能量",
				text = {
					"增加最左边的{C:attention}妖精{}属性小丑牌的",
					"{C:attention}加分{}及{C:money}金钱{}数值",
					"{C:inactive}(每张小丑牌可增强{C:attention}#1#{}{C:inactive}次)"
				}
			},
			c_poke_fighting_energy = {
				name = "斗能量",
				text = {
					"增加最左边的{C:attention}格斗{}属性小丑牌的",
					"{C:attention}加分{}及{C:money}金钱{}数值",
					"{C:inactive}(每张小丑牌可增强{C:attention}#1#{}{C:inactive}次)"
				}
			},
			c_poke_fire_energy = {
				name = "火能量",
				text = {
					"增加最左边的{C:attention}火{}属性小丑牌的",
					"{C:attention}加分{}及{C:money}金钱{}数值",
					"{C:inactive}(每张小丑牌可增强{C:attention}#1#{}{C:inactive}次)"
				}
			},
			c_poke_grass_energy = {
				name = "草能量",
				text = {
					"增加最左边的{C:attention}草{}属性小丑牌的",
					"{C:attention}加分{}及{C:money}金钱{}数值",
					"{C:inactive}(每张小丑牌可增强{C:attention}#1#{}{C:inactive}次)"
				}
			},
			c_poke_lightning_energy = {
				name = "电能量",
				text = {
					"增加最左边的{C:attention}电{}属性小丑牌的",
					"{C:attention}加分{}及{C:money}金钱{}数值",
					"{C:inactive}(每张小丑牌可增强{C:attention}#1#{}{C:inactive}次)"
				}
			},
			c_poke_metal_energy = {
				name = "钢能量",
				text = {
					"增加最左边的{C:attention}钢{}属性小丑牌的",
					"{C:attention}加分{}及{C:money}金钱{}数值",
					"{C:inactive}(每张小丑牌可增强{C:attention}#1#{}{C:inactive}次)"
				}
			},
			c_poke_psychic_energy = {
				name = "超能量",
				text = {
					"增加最左边的{C:attention}超能力{}属性小丑牌的",
					"{C:attention}加分{}及{C:money}金钱{}数值",
					"{C:inactive}(每张小丑牌可增强{C:attention}#1#{}{C:inactive}次)"
				}
			},
			c_poke_water_energy = {
				name = "水能量",
				text = {
					"增加最左边的{C:attention}水{}属性小丑牌的",
					"{C:attention}加分{}及{C:money}金钱{}数值",
					"{C:inactive}(每张小丑牌可增强{C:attention}#1#{}{C:inactive}次)"
				}
			}
		},
		Enhanced = {

		},
		Item = {
			c_poke_dragonscale = {
				name = "龙之鳞片",
				text = {
					"对最左边的小丑牌",
					"附上{C:dragon}龙{}贴纸",
					"产生一张{C:attention}皇帝{}牌",
					"{C:inactive}（必须有空位）{}"
				}
			},
			c_poke_dubious_disc = {
				name = "可疑修正档",
				text = {
					"随机化或移除",
					"所有手牌的{C:attention}加强效果{}",
					"{C:attention}进化牌{}",
					"{C:inactive}（包括无加强的牌）{}"
				}
			},
			c_poke_firestone = {
				name = "火之石",
				text = {
					"产生一张{C:attention}恋人{}牌",
					"{C:attention}此牌可用于进化{}",
					"{C:inactive}（必须有空位）{}"
				}
			},
			c_poke_greatball = {
				name = "超级球",
				text = {
					"产生一张随机",
					"{C:attention}一次进化的宝可梦小丑{}牌",
					"{C:inactive}（必须有空位）"
				}
			},
			c_poke_icestone = {
				name = "冰之石",
				text = {
					"产生一张{C:attention}正义{}牌",
					"{C:attention}此牌可用于进化{}",
					"{C:inactive}（必须有空位）{}"
				}
			},
			c_poke_kingsrock = {
				name = "王者之证",
				text = {
					"选择{C:attention}#1#{}张手牌",
					"并转换成{C:attention}K{}",
					"{C:attention}此牌可用于进化{}"
				}
			},
			c_poke_leafstone = {
				name = "叶之石",
				text = {
					"产生一张{C:attention}世界{}牌",
					"{C:attention}此牌可用于进化{}",
					"{C:inactive}（必须有空位）{}"
				}
			},
			c_poke_leek = {
				name = "大葱",
				text = {
					"产生一张{C:attention}幸运之轮{}牌",
					"{C:attention}携带物品牌{}",
					"{C:inactive}（必须有空位）{}"
				}
			},
			c_poke_leftovers = {
				name = "吃剩的东西",
				text = {
					"产生一张{C:attention}吊人{}牌",
					"{C:attention}携带物品牌{}",
					"{C:inactive}（必须有空位）{}"
				}
			},
			c_poke_linkcable = {
				name = "联系绳",
				text = {
					"产生一张{C:attention}死神{}牌",
					"{C:attention}此牌可用于进化{}",
					"{C:inactive}（必须有空位）{}"
				}
			},
			c_poke_metalcoat = {
				name = "金属膜",
				text = {
					"对最左边的小丑牌",
					"附上{C:metal}钢{}贴纸",
					"产生一张{C:attention}战车{}牌",
					"{C:inactive}（必须有空位）{}"
				}
			},
			c_poke_moonstone = {
				name = "月之石",
				text = {
					"产生一张{C:attention}月亮{}牌",
					"{C:attention}此牌可用于进化{}",
					"{C:inactive}（必须有空位）{}"
				}
			},
			c_poke_pokeball = {
				name = "精灵球",
				text = {
					"产生一张随机",
					"{C:attention}基础的宝可梦小丑{}牌",
					"{C:inactive}（必须有空位）"
				}
			},
			c_poke_shinystone = {
				name = "光之石",
				text = {
					"产生一张{C:attention}星星{}牌",
					"{C:attention}此牌可用于进化{}",
					"{C:inactive}（必须有空位）{}"
				}
			},
			c_poke_sunstone = {
				name = "日之石",
				text = {
					"产生一张{C:attention}太阳{}牌",
					"{C:attention}此牌可用于进化{}",
					"{C:inactive}（必须有空位）{}"
				}
			},
			c_poke_teraorb = {
				name = "太晶珠",
				text = {
					"对最左边的小丑牌",
					"附上随机的",
					"{C:pink}属性{}贴纸",
					"及{C:attention}+1{}{C:pink}能量{}"
				}
			},
			c_poke_thickclub = {
				name = "粗骨头",
				text = {
					"产生一张{C:attention}力量{}牌",
					"{C:attention}携带物品牌{}",
					"{C:inactive}（必须有空位）{}"
				}
			},
			c_poke_thunderstone = {
				name = "雷之石",
				text = {
					"产生一张{C:attention}恶魔{}牌",
					"{C:attention}此牌可用于进化{}",
					"{C:inactive}（必须有空位）{}"
				}
			},
			c_poke_twisted_spoon = {
				name = "弯曲的汤匙",
				text = {
					"产生比赛中最近使用的",
					"{C:item}物品{}牌或{C:pink}能量{}牌",
					"{s:0.8,C:item}弯曲的汤匙{s:0.8}除外"
				}
			},
			c_poke_upgrade = {
				name = "升级资料",
				text = {
					"选择{C:attention}#1#{}张手牌",
					"并附上随机的{C:attention}加强效果{}",
					"{C:attention}此牌可用于进化{}"
				}
			},
			c_poke_waterstone = {
				name = "水之石",
				text = {
					"产生一张{C:attention}教皇{}牌",
					"{C:attention}此牌可用于进化{}",
					"{C:inactive}（必须有空位）{}"
				}
			}
		},
		Joker = {
			j_poke_abra = {
				name = "凯西",
				text = {
					"如果打出的{C:attention}牌型{}",
					"已经在此局出过时",
					"有{C:green}#1#/#2#{}的几率产生一张{C:attention}愚者{}牌",
					"{C:inactive}（在{C:attention}#3#{}{C:inactive}个回合后进化）"
				}
			},
			j_poke_aerodactyl = {
				name = "化石翼龙",
				text = {
					"{C:attention}原始的#1#{}",
					"{C:attention}一张{}：{C:mult}+#2#{}倍数",
					"{C:attention}两张{} #1#：{C:mult}+#3#{}倍数及{C:chips}+#4#{}筹码",
					"{C:attention}三张{} #1#：{X:red,C:white}X#5#{}倍数",
					"{C:attention}四张或以上{} #1#：这张牌的加成{C:attention}翻倍"
				}
			},
			j_poke_alakazam = {
				name = "胡地",
				text = {
					"{C:attention}+#3#{}消耗牌字段",
					"如果打出的{C:attention}牌型{}",
					"已经在此局出过时",
					"有{C:green}#1#/#2#{}的几率产生一张{C:attention}愚者{}牌"
				}
			},
			j_poke_annihilape = {
				name = "弃世猴",
				text = {
					"每张计分的{C:attention}2{}，{C:attention}3{}，{C:attention}5{}或{C:attention}7{}",
					"给予{C:mult}+#1#{}倍数及{C:chips}+#2#{}筹码",
					"加成会在每次出牌叠加，回合结束后重设",
					"{C:inactive}（目前为{C:mult}+#3#{}{C:inactive}倍数{C:chips}+#4#{C:inactive}筹码）"
				}
			},
			j_poke_arbok = {
				name = "阿柏怪",
				text = {
					"如果打出的牌包含",
					"{C:attention}顺子{}，{C:mult}+#1#{}倍数",
					"如果也包含{C:attention}A{}",
					"产生一张{C:tarot}塔罗牌{}"
				}
			},
			j_poke_arcanine = {
				name = "风速狗",
				text = {
					"如果打出的牌包含{C:attention}同花{}",
					"{X:red,C:white} X#1# {}倍数",
					"选择{C:attention}盲注{}后",
					"产生一张{C:attention}恋人{}牌"
				}
			},
			j_poke_articuno = {
				name = "急冻鸟",
				text = {
					"每回合中的{C:attention}第一次出牌{}时",
					"新增{C:attention}银箔{}，随机的{C:attention}增强效果{}",
					"及随机的{C:attention}封蜡章{}到未计分的牌"
				}
			},
			j_poke_beedrill = {
				name = "大针蜂",
				text = {
					"{C:chips}+#1#{}筹码"
				}
			},
			j_poke_beldum = {
				name = "铁哑铃",
				text = {
					"如果打出的牌包含至少一张计分的{C:attention}A{}",
					"这张牌会得到{C:chips}+#2#{}筹码",
					"如果牌型是{C:attention}四条{}，得到的筹码{C:attention}翻倍{}",
					"{C:inactive}（目前为{C:chips}+#1#{}{C:inactive}筹码）",
					"{C:inactive}（在拥有{C:chips}+64{}{C:inactive}筹码时进化）"
				}
			},
			j_poke_bellossom = {
				name = "美丽花",
				text = {
					"每张计分的{C:attention}奇数{}牌",
					"会给予{C:mult}+#1#{}倍数或变成{C:attention}万能牌{}",
					"如果已经是{C:attention}万能牌{}，新增{C:dark_edition}彩色{}版本"
				}
			},
			j_poke_bellsprout = {
				name = "喇叭芽",
				text = {
					"每张计分的{C:attention}偶数{}点的牌",
					"给予{C:chips}+#1#{}筹码",
					"{C:inactive}（在{C:attention}#2#{}{C:inactive}个回合后进化）"
				}
			},
			j_poke_blastoise = {
				name = "水箭龟",
				text = {
					"{C:chips}+#3#{}出牌次数，{C:chips}+#1#{}筹码",
					"每个剩余的出牌次数给予{C:chips}+#2#{}筹码"
				}
			},
			j_poke_blaziken = {
				name = "火焰鸡",
				text = {
					"{C:mult}+#2#{}弃牌次数，{C:attention}多种性格{}",
					"这回合中，每张弃掉的{C:attention}#6#，#7#或#8#{}会给予{C:mult}+#4#{}倍数及{X:red,C:white} X#1# {}倍数",
					"如果你有其他的{X:fire,C:white}火{}属性或{X:earth,C:white}斗{}属性的牌，给予的倍数翻倍",
					"{C:inactive,s:0.8}（这包括小丑牌及能量牌）{}",
					"{C:inactive}（目前为{X:red,C:white} X#3# {}{C:inactive}倍数）{}"
				}
			},
			j_poke_blissey = {
				name = "幸福蛋",
				text = {
					"在每个回合中，头{C:attention}#1#{}张被触发的{C:attention}幸运牌{}",
					"会被复制并带有{C:dark_edition}彩色{}版本到牌组，",
					"然后抽到{C:attention}手牌{}中",
					"{C:inactive}（目前有{C:attention}#2#{C:inactive}/{C:attention}#1#{}{C:inactive}）"
				}
			},
			j_poke_boltund = {
				name = "逐电犬",
				text = {
					"如果打出的牌包含{C:attention}顺子{}",
					"给予{X:red,C:white}X#1#{}倍数及获得{C:money}$#2#{}"
				}
			},
			j_poke_buizel = {
				name = "泳圈鼬",
				text = {
					"每张在出牌中{C:attention}没有计分{}的牌",
					"会给予{C:chips}+#1#{}筹码",
					"{C:inactive}（在{C:attention}#2#{}{C:inactive}个回合后进化）"
				}
			},
			j_poke_bulbasaur = {
				name = "妙蛙种子",
				text = {
					"{C:attention}+#4#{}手牌数量",
					"手牌中，每一张{C:attention}#3#{}会给予{C:money}$#1#{}",
					"卡牌点数于每回合变更",
					"{C:inactive}（目前已获得{C:money}$#2#{C:inactive}）",
					"{C:inactive}（获得{C:money}$16{}{C:inactive}后进化）"
				}
			},
			j_poke_butterfree = {
				name = "巴大蝶",
				text = {
					"{C:mult}+#1#{}倍数"
				}
			},
			j_poke_caterpie = {
				name = "绿毛虫",
				text = {
					"{C:mult}+#1#{}倍数",
					"{C:inactive}（在{C:attention}#2#{}{C:inactive}个回合后进化）"
				}
			},
			j_poke_chansey = {
				name = "吉利蛋",
				text = {
					"每个回合中头{C:attention}#1#{}张被触发的{C:attention}幸运牌{}",
					"会增加一张复制的那张牌到牌组并抽到{C:attention}手牌{}中",
					"{C:inactive}（目前为{C:attention}#2#{C:inactive}/{C:attention}#1#{}{C:inactive}）",
					"{C:inactive}（在牌组有25%或以上是{C:attention}幸运牌{C:inactive}时进化）"
				}
			},
			j_poke_charizard = {
				name = "喷火龙",
				text = {
					"{C:red}+#4#{}次弃牌, {C:mult}+#1#{}倍数",
					"当剩下{C:attention}#3#{}次弃牌数时，倍数{X:mult,C:white} X#2# {}"
				}
			},
			j_poke_charjabug = {
				name = "虫电宝",
				text = {
					"每张你拥有的{X:lightning, C:black}电{}属性小丑牌",
					"会给予{C:mult}+#1#{}倍数{C:inactive}（包括自身）{}",
					"{C:inactive}（目前为{C:mult}#2#{C:inactive}倍数）",
					"{C:inactive}（使用{} {C:attention}雷之石{}{C:inactive}牌进化）"
				}
			},
			j_poke_charmander = {
				name = "小火龙",
				text = {
					"{C:red}+#4#{}次弃牌",
					"当剩下{C:attention}#3#{}次弃牌数时",
					"每次出牌会给此牌{C:mult}+#2#{}倍数",
					"{C:inactive}（目前为倍数{C:mult}+#1#{C:inactive}）",
					"{C:inactive}（在{C:mult}+16{}{C:inactive}倍数时进化）"
				}
			},
			j_poke_charmeleon = {
				name = "火恐龙",
				text = {
					"{C:red}+#4#{}次弃牌",
					"当剩下{C:attention}#3#{}次弃牌数时",
					"每次出牌会给此牌{C:mult}+#2#{}倍数",
					"{C:inactive}（目前为倍数{C:mult}+#1#{C:inactive}）",
					"{C:inactive}（在{C:mult}+36{}{C:inactive}倍数时进化）"
				}
			},
			j_poke_clefable = {
				name = "皮可西",
				text = {
					"每张计分的{C:clubs}#2#{}花色的牌",
					"会在这个回合中给予此牌{C:mult}+#1#{}倍数",
					"加成会依打出的{C:clubs}#2#{}花色牌的数量而增加",
					"{C:inactive,s:0.8}（如果打出了五张梅花牌，每张牌会给予{C:mult,s:0.8}+5{}{C:inactive,s:0.8}倍数）{}",
					"{C:inactive}(Currently {C:mult}#3#{}{C:inactive} Mult){}"
				}
			},
			j_poke_clefairy = {
				name = "皮皮",
				text = {
					"每张计分的",
					"{C:clubs}#2#{}花色的牌",
					"都给予{C:mult}+#1#{}倍数",
					"{C:inactive}（使用{}{C:attention}月之石{}{C:inactive}牌进化）"
				}
			},
			j_poke_cleffa = {
				name = "皮宝宝",
				text = {
					"{C:attention}婴儿{}，{X:red,C:white} X#1# {}倍数",
					"回合结束时",
					"产生一张带有{C:dark_edition}负片{}的{C:attention}月亮{}牌",
					"{C:inactive}（在{C:attention}#2#{}{C:inactive}个回合后进化）"
				}
			},
			j_poke_cloyster = {
				name = "刺甲贝",
				text = {
					"如果打出的牌有计分的{C:attention}5{}",
					"每一张有{C:green}#1#/#2#{}的几率重新触发"
				}
			},
			j_poke_combusken = {
				name = "力壮鸡",
				text = {
					"{C:mult}+#3#{}弃牌次数，{C:attention}多种性格{}",
					"这回合中，每张弃掉的{C:attention}#5#，#6#或#7#{}会给予{C:mult}+#1#{}倍数",
					"如果你有其他的{X:fire,C:white}火{}属性或{X:earth,C:white}斗{}属性的牌，给予的倍数翻倍",
					"{C:inactive,s:0.8}（这包括小丑牌及能量牌）{}",
					"{C:inactive}（目前为{C:mult}#4#{}{C:inactive}倍数）{}",
					"{C:inactive}（在给予{C:mult}#2#/150{}{C:inactive}倍数后进化）"
				}
			},
			j_poke_crobat = {
				name = "叉字蝠",
				text = {
					"{C:attention}随机{}所有计分牌现有的强化效果",
					"从{C:attention}倍数{}及{C:attention}万能{}牌获得{C:mult}+#2#{}倍数",
					"从{C:attention}奖励{}及{C:attention}石头{}牌获得{C:chips}+#4#{}筹码",
					"从{C:attention}钢铁{}及{C:attention}玻璃{}牌获得{X:red,C:white}X#6#{}倍数",
					"从{C:attention}黄金{}及{C:attention}幸运{}牌获得{C:money}$#8#{}金钱",
					"{C:inactive}（目前为{C:mult}+#1#{}，{C:chips}+#3#{}，{X:red,C:white}X#5#{}，{C:inactive}回合结束时，{}{C:money}$#7#{}）"
				}
			},
			j_poke_cubone = {
				name = "卡拉卡拉",
				text = {
					"当你得到这张牌后",
					"产生一张{C:attention}粗骨头{}牌",
					"每个已占据的消耗牌字段",
					"给予{C:mult}+#1#{}倍数",
					"{C:inactive,s:0.75}（{C:attention,s:0.75}粗骨头{}{C:inactive,s:0.75}牌当作双倍）{}",
					"{C:inactive}（目前为{C:mult}+#3#{C:inactive}倍数）",
					"{C:inactive}（在{C:attention}#2#{}{C:inactive}个回合后进化）"
				}
			},
			j_poke_dachsbun = {
				name = "麻花犬",
				text = {
					"如果打出的牌包含{C:attention}#3#{}，此牌获得{C:chips}+#2#{}筹码",
					"所需的{C:attention}卡牌点数{}会在每次触发时提升一点",
					"每张{X:fire,C:white}火{}属性小丑牌会增加{C:chips}+1{}到获得的筹码",
					"{C:inactive,s:0.8}（如果是A，那就会变回2）",
					"{C:inactive}（目前有{C:chips}+#1#{C:inactive}筹码）"
				}
			},
			j_poke_delibird = {
				name = "信使鸟",
				text = {
					"回合结束后，",
					"获得一个{S:1.1,C:green,E:2}礼物{}",
					"{C:inactive}（必须有空位）"
				}
			},
			j_poke_dewgong = {
				name = "白海狮",
				text = {
					"在{C:attention}第一次{}出牌中",
					"新增随机的{C:attention}封蜡章{}到最左边的计分牌"
				}
			},
			j_poke_diglett = {
				name = "地鼠",
				text = {
					"如果打出的牌包含{C:attention}三条{}",
					"给予{C:chips}+#2#{}筹码",
					"如果打出的牌包含{C:attention}2{}，{C:attention}3{}或{C:attention}4{}",
					"给予{C:mult}+#3#{}倍数",
					"{C:inactive}（在{C:attention}#1#{}{C:inactive}个回合后进化）"
				}
			},
			j_poke_ditto = {
				name = "百变怪",
				text = {
					"卖出这张牌来产生最左边的小丑牌的副本",
					"副本带有{C:attention}非保久{}贴纸及{C:colorless}一般{}贴纸",
					"{C:inactive}（移除永恒贴纸，不能复制其他百变怪）{}"
				}
			},
			j_poke_dodrio = {
				name = "嘟嘟利",
				text = {
					"如果计分的牌包含",
					"至少{C:attention}3{}张人头牌或{C:attention}葫芦",
					"给予{C:mult}+#1#{}倍数",
					"如果包含两者，加成效果叠加"
				}
			},
			j_poke_doduo = {
				name = "嘟嘟",
				text = {
					"如果计分的牌包含",
					"至少{C:attention}2{}张人头牌或{C:attention}葫芦",
					"给予{C:mult}+#1#{}倍数",
					"如果包含两者，加成效果叠加",
					"{C:inactive}（在{C:attention}#2#{}{C:inactive}个回合后进化）"
				}
			},
			j_poke_dragonair = {
				name = "哈克龙",
				text = {
					"如果打出的牌不超过{C:attention}#3#{}张",
					"这张牌会获得{C:mult}+#2#{}倍数",
					"{C:inactive}（目前为{C:mult}+#1#{C:inactive}倍数）",
					"{C:inactive}（在拥有{C:mult}+30{}{C:inactive}倍数时进化）"
				}
			},
			j_poke_dragonite = {
				name = "快龙",
				text = {
					"{C:mult}+#1#{}倍数",
					"如果只打出一张牌",
					"重新触发那张牌{C:attention}#2#{}次"
				}
			},
			j_poke_dratini = {
				name = "迷你龙",
				text = {
					"如果打出的牌不超过{C:attention}#3#{}张",
					"这张牌会获得{C:mult}+#2#{}倍数",
					"{C:inactive}（目前为{C:mult}+#1#{C:inactive}倍数）",
					"{C:inactive}（在拥有{C:mult}+10{}{C:inactive}倍数时进化）"
				}
			},
			j_poke_drowzee = {
				name = "催眠貘",
				text = {
					"在此局中，每一张已使用并不重复的",
					"{C:planet}行星牌{}给予{C:mult}+#2#{}倍数",
					"{C:inactive}（目前为{C:mult}+#1#{C:inactive}倍数）",
					"{C:inactive}（在{C:mult}+21{C:inactive}倍数时进化）"
				}
			},
			j_poke_dugtrio = {
				name = "三地鼠",
				text = {
					"如果打出的牌包含{C:attention}三条{}",
					"给予{C:chips}+#2#{}筹码",
					"如果打出的牌包含{C:attention}2{}，{C:attention}3{}或{C:attention}4{}",
					"给予{X:red,C:white} X#1# {}倍数"
				}
			},
			j_poke_eevee = {
				name = "伊布",
				text = {
					"{C:attention}头五次{}的{C:green}重铩{}",
					"会获得{C:money}$#1#{}",
					"{C:inactive}（每种的进化之石会有不同的进化）{}",
					"{C:inactive}目前为{C:attention}#2#{C:inactive}/#3#"
				}
			},
			j_poke_ekans = {
				name = "阿柏蛇",
				text = {
					"如果打出的牌包含",
					"{C:attention}顺子{}，{C:mult}+#1#{}倍数",
					"{C:inactive}（在{C:attention}#2#{}{C:inactive}个回合后进化）"
				}
			},
			j_poke_electabuzz = {
				name = "电击兽",
				text = {
					"当回合结束时，每次{C:attention}卖出{}一张牌",
					"会把此牌的{C:attention}出售价值{}提升{C:money}$#1#{}",
					"{C:inactive}（使用{}{C:attention}联系绳{}{C:inactive}牌进化）"
				}
			},
			j_poke_electivire = {
				name = "电击魔兽",
				text = {
					"当回合结束时，每次{C:attention}卖出{}一张牌",
					"会把此牌的{C:attention}出售价值{}提升{C:money}$#1#{}",
					"此牌的出售价值每一元会给予{X:mult,C:white}X#2#{}倍数",
					"{C:inactive}（目前为{X:mult,C:white}X#3#{}{C:inactive}倍数）"
				}
			},
			j_poke_electrode = {
				name = "顽皮雷弹",
				text = {
					"如果这张牌是{C:attention}最左边{}的小丑牌",
					"给予{C:mult}+#1#{}倍数及{C:money}$#2#{}",
					"并减益这张小丑牌直到回合结束"
				}
			},
			j_poke_elekid = {
				name = "电击怪",
				text = {
					"{C:attention}婴儿{}，{X:red,C:white} X#1# {}倍数",
					"当这个小丑牌进化时",
					"产生一个{C:attention}优惠券{}标签",
					"{C:inactive}（你没看错，这会{C:attention}减低{C:inactive}你的倍数）",
					"{C:inactive}（在{C:attention}#2#{}{C:inactive}个回合后进化）"
				}
			},
			j_poke_espeon = {
				name = "太阳伊布",
				text = {
					"每{C:attention}三{}次的{C:green}重铩{}",
					"会产生一张{C:attention}太阳{}牌",
					"在{C:attention}第一次出牌{}中，重新触发所有打出的",
					"{C:hearts}红心{}花色的牌",
					"{C:inactive}（必须有空位）",
					"{C:inactive}（目前{C:attention}#1#{}{C:inactive}/3次重铩）"
				}
			},
			j_poke_everstone = {
				name = "不变之石",
				text = {
					"宝可梦{C:attention}不能{}进化",
					"每张{C:attention}基础{}宝可梦会给予{X:mult,C:white} X#1# {}倍数"
				}
			},
			j_poke_exeggcute = {
				name = "蛋蛋",
				text = {
					"每张计分的{C:hearts}#2#{}花色的牌",
					"给予{C:mult}+#1#{}倍数",
					"{C:inactive}（使用{}{C:attention}叶之石{}{C:inactive}牌进化）"
				}
			},
			j_poke_exeggutor = {
				name = "椰蛋树",
				text = {
					"每张计分的{C:hearts}#3#{}花色的牌",
					"有{C:green}#4#/#5#{}的几率",
					"给予{C:mult}+#1#{}倍数及{X:mult,C:white}X#2#{}倍数"
				}
			},
			j_poke_farfetchd = {
				name = "大葱鸭",
				text = {
					"当你得到这张牌后",
					"产生一张{C:attention}大葱{}牌",
					"有{C:green}#2#/#3#{}的几率给予{X:red,C:white}X#1#{}倍数",
					"每张在手的{C:attention}大葱{}牌加倍这个几率"
				}
			},
			j_poke_fearow = {
				name = "大嘴雀",
				text = {
					"当有{C:attention}#1#{}张牌计分后",
					"升级下一个打出的{C:attention}牌型{}",
					"{C:inactive}（目前有{C:attention}#2#{}{C:inactive}/#1#）{C:inactive}#3#{}"
				}
			},
			j_poke_fidough = {
				name = "狗仔包",
				text = {
					"如果打出的牌包含{C:attention}#3#{}，此牌获得{C:chips}+#2#{}筹码",
					"所需的{C:attention}卡牌点数{}会在每次触发时提升一点",
					"{C:inactive,s:0.8}（如果是A，那就会变回2）",
					"{C:inactive}（目前有{C:chips}+#1#{C:inactive}筹码）",
					"{C:inactive}（在拥有{X:fire,C:white}火{}{C:inactive}属性小丑牌时进化）"
				}
			},
			j_poke_flareon = {
				name = "火伊布",
				text = {
					"每{C:attention}三{}次的{C:green}重铩{}",
					"会给此牌{X:red,C:white} X#2# {}倍数",
					"{C:inactive}（目前有{C:attention}#3#{}{C:inactive}/3次重铩）",
					"{C:inactive}（目前为{X:red,C:white} X#1# {}{C:inactive}筹码）"
				}
			},
			j_poke_floatzel = {
				name = "浮潜鼬",
				text = {
					"每张在出牌中{C:attention}没有计分{}的牌",
					"会给予{C:chips}+#1#{}筹码"
				}
			},
			j_poke_furret = {
				name = "大尾立",
				text = {
					"如果打出的牌型不是上一个的牌型",
					"这张牌会获得{C:mult}+#2#{}倍数",
					"{C:inactive}（上一个牌型：{C:attention}#3#{}{C:inactive}）",
					"{C:inactive}（目前为{C:mult}+#1#{} {C:inactive}倍数）"
				}
			},
			j_poke_gastly = {
				name = "鬼斯",
				text = {
					"有{C:green}#1#/#2#{}的几率",
					"对随机的{C:attention}小丑牌{}",
					"更换至{C:dark_edition}负片{}效果",
					"{S:1.1,C:red,E:2}触发后自我毁灭{}",
					"{C:inactive}（在{C:attention}#3#{}{C:inactive}个回合后进化）"
				}
			},
			j_poke_gengar = {
				name = "耿鬼",
				text = {
					"{C:green}百份比几率",
					"回合结束后，有几率",
					"对随机的{C:attention}小丑牌{}",
					"更换至{C:dark_edition}负片{}效果",
					"{C:inactive,s:0.8}（自身除外）{}"
				}
			},
			j_poke_geodude = {
				name = "小拳石",
				text = {
					"{C:chips}+#1#{}筹码",
					"{C:attention}-#2#{}手牌数量",
					"{C:inactive}（在{C:attention}#3#{}{C:inactive}个回合后进化）"
				}
			},
			j_poke_glaceon = {
				name = "冰伊布",
				text = {
					"每{C:attention}三{}次的{C:green}重铩{}",
					"会产生一张随机的{C:attention}塔罗牌{}",
					"有{C:green}#2#{}/{C:green}#3#{}的几率产生两张{C:attention}塔罗牌{}",
					"{C:inactive}（必须有空位）",
					"{C:inactive}（目前{C:attention}#1#{}{C:inactive}/3次重铩）"
				}
			},
			j_poke_gloom = {
				name = "臭臭花",
				text = {
					"每张计分的{C:attention}奇数{}点的牌",
					"都给予{C:mult}+#1#{}或{C:mult}+#2#{}倍数",
					"{C:inactive}（使用{}{C:attention}叶之石{}{C:inactive}或{}{C:attention}日之石{}{C:inactive}牌进化）"
				}
			},
			j_poke_golbat = {
				name = "大嘴蝠",
				text = {
					"移除计分牌的加强效果",
					"从{C:attention}倍数{}及{C:attention}万能{}牌获得{C:mult}+#2#{}倍数",
					"从{C:attention}奖励{}及{C:attention}石头{}牌获得{C:chips}+#4#{}筹码",
					"从{C:attention}钢铁{}及{C:attention}玻璃{}牌获得{X:red,C:white}X#6#{}倍数",
					"从{C:attention}黄金{}及{C:attention}幸运{}牌获得{C:money}$#8#{}金钱",
					"{C:inactive}（在移除{C:attention}#9#{}{C:inactive}/20张加强牌时进化）",
					"{C:inactive}（目前为{C:mult}+#1#{}，{C:chips}+#3#{}，{X:red,C:white}X#5#{}，{C:inactive}回合结束时，{}{C:money}$#7#{}）"
				}
			},
			j_poke_goldeen = {
				name = "角金鱼",
				text = {
					"重新触发在手牌中所有的",
					"{C:attention}黄金牌{}一次",
					"{C:inactive}（在{C:attention}#2#{}{C:inactive}个回合后进化）"
				}
			},
			j_poke_golduck = {
				name = "哥达鸭",
				text = {
					"如果打出的牌为",
					"单张{C:attention}人头牌{}，获得{C:money}$#1#{}",
					"并变为{C:attention}黄金牌{}"
				}
			},
			j_poke_golem = {
				name = "隆隆岩",
				text = {
					"{C:chips}+#1#{}筹码",
					"{C:attention}-#2#{}手牌数量"
				}
			},
			j_poke_graveler = {
				name = "隆隆石",
				text = {
					"{C:chips}+#1#{}筹码",
					"{C:attention}-#2#{}手牌数量",
					"{C:inactive}（使用{}{C:attention}联系绳{}{C:inactive}牌进化）"
				}
			},
			j_poke_grimer = {
				name = "臭泥",
				text = {
					"如果你的牌组有多于{C:attention}#3#{}张牌",
					"给予{C:mult}+#1#{}倍数",
					"每回合结束时",
					"增加一张随机的扑克牌到牌组",
					"{C:inactive}（在{C:attention}#2#{}{C:inactive}个回合后进化）"
				}
			},
			j_poke_grovyle = {
				name = "森林蜥蜴",
				text = {
					"{C:attention}+#3#{}手牌数量，{C:attention}多种性格{}",
					"打出的{C:attention}#6#，#7#或#8#{}牌",
					"会有{C:green}#4#/#5#{}的几率获得{C:money}$#1#{}",
					"如果你有其他的{X:grass,C:white}草{}属性牌，效果确保触发",
					"{C:inactive,s:0.8}（这包括小丑牌及能量牌）{}",
					"{C:inactive}在赚取{C:money}$#2#/32{}后进化）"
				}
			},
			j_poke_growlithe = {
				name = "卡蒂狗",
				text = {
					"如果打出的牌包含{C:attention}同花{}",
					"{C:mult}+#1#{}倍数",
					"{C:inactive}（使用{}{C:attention}火之石{}{C:inactive}牌进化）"
				}
			},
			j_poke_grubbin = {
				name = "强颚鸡母虫",
				text = {
					"{C:mult}+#1#{}倍数",
					"如果你有{X:lightning, C:black}电{}属性的小丑牌",
					"此牌给予的倍数会增加{C:attention}三倍{}",
					"{C:inactive}（在{C:attention}#2#{}{C:inactive}个回合后进化）"
				}
			},
			j_poke_gyarados = {
				name = "暴鲤龙",
				text = {
					"{X:red,C:white} X#1# {}倍数"
				}
			},
			j_poke_happiny = {
				name = "小福蛋",
				text = {
					"{C:attention}婴儿{}，{X:red,C:white} X#1# {}倍数",
					"如果这张牌是最右边的小丑牌",
					"在{C:attention}最后一次{}出牌时，所有计分牌会变成{C:attention}幸运牌{}",
					"{C:inactive}（在{C:attention}#2#{}{C:inactive}个回合后进化）"
				}
			},
			j_poke_haunter = {
				name = "鬼斯通",
				text = {
					"有{C:green}#1#/#2#{}的几率",
					"对随机的{C:attention}小丑牌{}",
					"更换至{C:dark_edition}负片{}效果",
					"{S:1.1,C:red,E:2}触发后自我毁灭{}",
					"{C:inactive}（使用{}{C:attention}联系绳{}{C:inactive}牌进化）"
				}
			},
			j_poke_hitmonchan = {
				name = "快拳郎",
				text = {
					"在你的牌组中，每张多于{C:attention}#2#{}张的牌",
					"都给予{X:red,C:white}X#1#{}倍数",
					"{C:inactive}（目前为{X:red,C:white}X#3#{C:inactive}倍数）"
				}
			},
			j_poke_hitmonlee = {
				name = "飞腿郎",
				text = {
					"在你的牌组中，每张少于{C:attention}#2#{}张的牌",
					"都给予{X:red,C:white}X#1#{}倍数",
					"{C:inactive}（目前为{X:red,C:white}X#3#{C:inactive}倍数）"
				}
			},
			j_poke_hitmontop = {
				name = "战舞郎",
				text = {
					"{X:red,C:white} X#1# {}倍数",
					"选择盲注时",
					"如果你的牌组正好有{C:attention}#2#{}张牌",
					"这张牌会获得{X:red,C:white} X#3# {}倍数"
				}
			},
			j_poke_horsea = {
				name = "墨海马",
				text = {
					"每张计分的{C:attention}6{}",
					"会给此牌{C:mult}+#2#{}倍数",
					"{C:inactive}（目前为{C:mult}+#1#{C:inactive}倍数）",
					"{C:inactive}（{C:mult}+12{} {C:inactive}倍数时进化）"
				}
			},
			j_poke_hypno = {
				name = "引梦貘人",
				text = {
					"在此局中，每一张已使用并不重复的",
					"{C:planet}行星牌{}给予{C:mult}+#2#{}倍数",
					"当你得到这张牌后",
					"产生一张{C:spectral}入迷{}牌",
					"{C:inactive}（目前为{C:mult}+#1#{C:inactive}倍数）"
				}
			},
			j_poke_igglybuff = {
				name = "宝宝丁",
				text = {
					"{C:attention}婴儿{}，{X:red,C:white} X#1# {}倍数",
					"回合结束时",
					"产生一张带有{C:dark_edition}负片{}的{C:attention}世界{}牌",
					"{C:inactive}（在{C:attention}#2#{}{C:inactive}个回合后进化）"
				}
			},
			j_poke_ivysaur = {
				name = "妙蛙草",
				text = {
					"{C:attention}+#3#{}手牌数量",
					"手牌中，每一张{C:attention}#4#{}会给予{C:money}$#1#{}或{C:money}$#5#{}",
					"卡牌点数于每回合变更",
					"{C:inactive}（目前已获得{C:money}$#2#{C:inactive}）",
					"{C:inactive}（获得{C:money}$16{}{C:inactive}后进化）"
				}
			},
			j_poke_jelly_donut = {
				name = "御握",
				text = {
					"选择盲注后",
					"产生一张{C:colorless}无色{}{C:pink}能量{}牌",
					"{C:inactive}（剩余{C:attention}#1#{}{C:inactive}个回合）{}"
				}
			},
			j_poke_jigglypuff = {
				name = "胖丁",
				text = {
					"每张计分的",
					"{C:spades}#2#{}花色的牌",
					"都给予{C:mult}+#1#{}倍数",
					"{C:inactive}（使用{}{C:attention}月之石{}{C:inactive}牌进化）"
				}
			},
			j_poke_jolteon = {
				name = "雷伊布",
				text = {
					"每{C:attention}三{}次的{C:green}重铩{}",
					"会给予{C:money}$#1#{}",
					"{C:inactive}（目前有{C:attention}#2#{}{C:inactive}/3次重铩）"
				}
			},
			j_poke_jynx = {
				name = "迷唇姐",
				text = {
					"从{C:attention}商店{}，{C:attention}标准{}礼包，",
					"{C:spectral}传说动物{}及特定的小丑牌",
					"加入到牌组的{C:attention}扑克牌{}会被{C:attention}复制{}"
				}
			},
			j_poke_kabuto = {
				name = "化石盔",
				text = {
					"{C:attention}原始的#1#{}",
					"{C:attention}一张{}：{C:chips}+#2#{}筹码",
					"{C:attention}两张{}：计分的{C:attention}#1#{}可永久获得{C:chips}+#3#{}筹码",
					"{C:attention}三张或以上{}：{C:chips}+#4#{}筹码",
					"{C:inactive}（在触发第三级{C:attention}#5#{}{C:inactive}/#6#次后进化）"
				}
			},
			j_poke_kabutops = {
				name = "镰刀盔",
				text = {
					"{C:attention}原始的#1#{}",
					"{C:attention}一张{}：{C:chips}+#2#{}筹码",
					"{C:attention}两张{}：计分的{C:attention}#1#{}可永久获得{C:chips}+#3#{}筹码",
					"{C:attention}三张{}：{C:chips}+#4#{}筹码",
					"{C:attention}四张或以上{}：重新触发{C:attention}头两张{}打出的牌{C:attention}#5#{}次"
				}
			},
			j_poke_kadabra = {
				name = "勇基拉",
				text = {
					"如果打出的{C:attention}牌型{}",
					"已经在此局出过时",
					"有{C:green}#1#/#2#{}的几率产生一张{C:attention}愚者{}牌",
					"{C:inactive}（使用{}{C:attention}联系绳{}{C:inactive}牌进化）"
				}
			},
			j_poke_kakuna = {
				name = "铁壳蛹",
				text = {
					"{C:chips}+#1#{}筹码",
					"{C:inactive}（在{C:attention}#2#{}{C:inactive}个回合后进化）"
				}
			},
			j_poke_kangaskhan = {
				name = "袋兽",
				text = {
					"{C:attention}+#1#{}消耗牌字段",
					"{C:chips}-#2#{}出牌次数"
				}
			},
			j_poke_kingdra = {
				name = "刺龙王",
				text = {
					"每张计分的{C:attention}6{}",
					"会给此牌{C:mult}+#2#{}倍数",
					"手牌中的每张{C:attention}K{}",
					"在{C:attention}出牌{}时会给此牌{X:red,C:white}X#4#{}倍数",
					"{C:inactive}（目前为{C:mult}+#1#{C:inactive}倍数，{X:red,C:white}X#3#{}倍数{C:inactive}）"
				}
			},
			j_poke_kingler = {
				name = "巨钳蟹",
				text = {
					"每张计分的{C:attention}人头牌{}",
					"变成{C:attention}奖励牌{}"
				}
			},
			j_poke_koffing = {
				name = "瓦斯弹",
				text = {
					"卖出这张牌来减低",
					"目前{C:attention}Boss 盲注{}所需的一半分数",
					"{C:inactive}（在{C:attention}#1#{}{C:inactive}个回合后进化）"
				}
			},
			j_poke_krabby = {
				name = "大钳蟹",
				text = {
					"每张计分的{C:attention}人头牌{}",
					"给予{C:chips}+#1#{}筹码",
					"{C:inactive}（在{C:attention}#2#{}{C:inactive}个回合后进化）"
				}
			},
			j_poke_lapras = {
				name = "拉普拉斯",
				text = {
					"本比赛中每个已跳过的{C:attention}盲注{}",
					"会给此牌{C:chips}+#2#{}筹码",
					"{C:inactive}（目前为{C:chips}+#1# {C:inactive}筹码）"
				}
			},
			j_poke_leafeon = {
				name = "叶伊布",
				text = {
					"每{C:attention}三{}次的{C:green}重铩{}",
					"会产生一张{C:attention}世界{}牌",
					"每张计分的{C:spades}#2#{}花色的牌",
					"可永久获得{C:chips}+#3#{}筹码",
					"{C:inactive}（必须有空位）",
					"{C:inactive}（目前{C:attention}#1#{}{C:inactive}/3次重铩）"
				}
			},
			j_poke_lickilicky = {
				name = "大舌舔",
				text = {
					"头两张计分的{C:attention}J{}会给予{X:mult,C:white} X#1# {}倍数",
					"之后每张计分的{C:attention}J{}会给予{X:mult,C:white} X#2# {}倍数"
				}
			},
			j_poke_lickitung = {
				name = "大舌头",
				text = {
					"头两张的计分的{C:attention}J{}",
					"给予{X:mult,C:white} X#1# {}倍数",
					"{C:inactive}（在触发{C:attention}#2#{}{C:inactive}/20 次后进化）{}"
				}
			},
			j_poke_machamp = {
				name = "怪力",
				text = {
					"{C:chips}+#1#{}出牌次数",
					"{C:mult}-#2#弃牌次数{}",
					"{C:mult}+#3#{}倍数"
				}
			},
			j_poke_machoke = {
				name = "豪力",
				text = {
					"{C:chips}+#1#{}出牌次数",
					"{C:mult}-#2#弃牌次数{}",
					"{C:mult}+#3#{}倍数",
					"{C:inactive}（使用{}{C:attention}联系绳{}{C:inactive}牌进化）"
				}
			},
			j_poke_machop = {
				name = "腕力",
				text = {
					"{C:chips}+#1#{}出牌次数",
					"{C:mult}-#2#弃牌次数{}",
					"{C:mult}+#4#{}倍数",
					"{C:inactive}（在{C:attention}#3#{}{C:inactive}个回合后进化）"
				}
			},
			j_poke_magby = {
				name = "鸭嘴宝宝",
				text = {
					"{C:attention}婴儿{}，{X:red,C:white} X#1# {}倍数",
					"{C:red}+#2#{}弃牌次数",
					"{C:inactive}（你没看错，这会{C:attention}减低{C:inactive}你的倍数）",
					"{C:inactive}（在{C:attention}#3#{}{C:inactive}个回合后进化）"
				}
			},
			j_poke_magikarp = {
				name = "鲤鱼王",
				text = {
					"附上{C:attention}水花喷溅{}效果",
					"{C:inactive}（在{C:attention}#1#{}{C:inactive}个回合后进化）"
				}
			},
			j_poke_magmar = {
				name = "鸭嘴火兽",
				text = {
					"如果回合中的{C:attention}第一次{}弃牌只有{C:attention}一{}张牌",
					"摧毁它并获得{C:mult}+#2#{}倍数",
					"{C:inactive}（目前为{C:mult}+#1#{C:inactive}倍数）",
					"{C:inactive}（使用{}{C:attention}联系绳{}{C:inactive}牌进化）"
				}
			},
			j_poke_magmortar = {
				name = "鸭嘴炎兽",
				text = {
					"如果回合中的{C:attention}第一次{}弃牌只有{C:attention}一{}张牌",
					"摧毁它并获得{C:mult}+#2#{}倍数",
					"每张弃掉的牌会给予{X:mult,C:white}X#4#{}倍数",
					"{C:inactive}（目前为{C:mult}+#1#{C:inactive}倍数，{X:mult,C:white}X#3#{}{C:inactive}倍数）"
				}
			},
			j_poke_magnemite = {
				name = "小磁怪",
				text = {
					"每张计分的{C:attention}钢铁{}牌",
					"给予{X:red,C:white}X#1#{}倍数",
					"{C:inactive}（在{C:attention}#2#{}{C:inactive}个回合后进化）"
				}
			},
			j_poke_magneton = {
				name = "三合一磁怪",
				text = {
					"每张计分的{C:attention}钢铁{}牌给予{X:red,C:white}X#1#{}倍数",
					"给予{X:red,C:white}X#1#{}倍数",
					"每张在旁边的{X:metal,C:white}钢{}属性小丑牌",
					"会对加成增加{X:red,C:white}X#2#{}倍数",
					"{C:inactive}（目前为{X:red,C:white}X#3#{}{C:inactive}倍数）{}",
					"{C:inactive}（使用{C:attention}雷之石{}{C:inactive}牌进化）"
				}
			},
			j_poke_magnezone = {
				name = "自爆磁怪",
				text = {
					"每张计分的{C:attention}钢铁牌{}",
					"会给予{X:red,C:white}X#1#{}倍数",
					"每张在旁边的{X:metal,C:white}钢{}属性小丑牌",
					"会给予{X:red,C:white}X#2#{}倍数"
				}
			},
			j_poke_mankey = {
				name = "猴怪",
				text = {
					"每张计分的{C:attention}2{}，",
					"{C:attention}3{}或{C:attention}5{}给予",
					"{C:mult}+#1#{}倍数及{C:chips}+#2#{}筹码",
					"{C:inactive}（在{C:attention}#3#{}{C:inactive}个回合后进化）"
				}
			},
			j_poke_marowak = {
				name = "嘎啦嘎啦",
				text = {
					"{C:attention}+#2#{}消耗牌字段",
					"每个已占据的消耗牌字段",
					"给予{X:mult,C:white} X#1# {}倍数",
					"{C:inactive,s:0.75}（{C:attention,s:0.75}粗骨头{}{C:inactive,s:0.75}牌当作双倍）{}",
					"{C:inactive}（目前为{X:mult,C:white} X#3# {}{C:inactive}倍数）"
				}
			},
			j_poke_marshtomp = {
				name = "沼跃鱼",
				text = {
					"{C:chips}+#3#{}出牌次数，{C:attention}多种性格{}",
					"打出的{C:attention}#4#，#5#或#6#{}会给予{C:chips}+#1#{}筹码",
					"如果你有其他的{X:water,C:white}水{}属性或{X:earth,C:white}地{}属性的牌，给予的筹码翻倍",
					"{C:inactive,s:0.8}（这包括小丑牌及能量牌）{}",
					"{C:inactive}（在给予{C:chips}#2#/960{}{C:inactive}筹码后进化）"
				}
			},
			j_poke_meowth = {
				name = "喵喵",
				text = {
					"每回合结束时",
					"获得{C:money}$#1#{}",
					"{C:inactive}（在{C:attention}#2#{}{C:inactive}个回合后进化）"
				}
			},
			j_poke_metagross = {
				name = "巨金怪",
				text = {
					"如果打出的牌型是{C:attention}四条{}",
					"给予{C:chips}+#1#{}筹码",
					"每张打出的牌会给予等同那张牌的",
					"基本筹码的{C:attention}立方根{}的{X:mult,C:white}X{}倍数"
				}
			},
			j_poke_metang = {
				name = "金属怪",
				text = {
					"如果打出的牌包含至少两张计分的{C:attention}A{}",
					"这张牌会得到{C:chips}+#2#{}筹码",
					"如果牌型是{C:attention}四条{}，得到的筹码有{C:attention}四倍{}加成",
					"{C:inactive}（目前为{C:chips}+#1#{}{C:inactive}筹码）",
					"{C:inactive}（在拥有{C:chips}+256{}{C:inactive}筹码时进化）"
				}
			},
			j_poke_metapod = {
				name = "铁甲蛹",
				text = {
					"{C:mult}+#1#{}倍数",
					"{C:inactive}（在{C:attention}#2#{}{C:inactive}个回合后进化）"
				}
			},
			j_poke_mew = {
				name = "梦幻",
				text = {
					"离开商店后，产生一张随机的",
					"{C:dark_edition}负片{}{C:attention}塔罗{}，{C:spectral}幻灵{}或{C:item}物品{}牌",
					"有几率产生一张随机的{C:dark_edition}负片{}小丑牌"
				}
			},
			j_poke_mewtwo = {
				name = "超梦",
				text = {
					"离开商店后，{C:attention}复制及摧毁{}最左边的{C:attention}小丑牌{}",
					"副本带有{C:dark_edition}彩色{}版本及{C:attention}+1{}{C:pink}能量{}",
					"每张{C:dark_edition}彩色{}版本的小丑牌给予{X:mult,C:white} X#1# {}倍数",
					"{C:inactive}（不能摧毁自身）"
				}
			},
			j_poke_mimejr = {
				name = "魔尼尼",
				text = {
					"{C:attention}婴儿{}，{X:red,C:white} X#1# {}倍数",
					"回合结束后，新增{C:attention}红色{}或{C:attention}蓝色{}封蜡章",
					"到牌组中的随机一张牌",
					"{C:inactive}（在{C:attention}#2#{}{C:inactive}个回合后进化）"
				}
			},
			j_poke_mimikyu = {
				name = "谜拟Ｑ",
				text = {
					"如果打出的牌没有计分的{C:hearts}#2#{}牌",
					"给予{C:chips}+#1#{}筹码",
					"如果你得到最少{C:attention}50%{}的所需筹码",
					"此牌会防止死亡",
					"{C:inactive}(#3#){}"
				}
			},
			j_poke_missingno = {
				name = "Missingno.",
				text = {
					"得到这张牌时，产生消耗牌",
					"回合结束后，产生{C:attention}#1#{}个{C:attention}标签{}"
				}
			},
			j_poke_moltres = {
				name = "火焰鸟",
				text = {
					"升级每回合中",
					"{C:attention}头三次弃牌{}",
					"的牌型等级"
				}
			},
			j_poke_mrmime = {
				name = "魔墙人偶",
				text = {
					"重新触发在手牌中",
					"最左边的牌{C:attention}#1#{}次"
				}
			},
			j_poke_mudkip = {
				name = "水跃鱼",
				text = {
					"{C:chips}+#3#{}出牌次数，{C:attention}多种性格{}",
					"打出的{C:attention}#4#，#5#或#6#{}会给予{C:chips}+#1#{}筹码",
					"如果你有其他的{X:water,C:white}水{}属性或{X:earth,C:white}地{}属性的牌，给予的筹码翻倍",
					"{C:inactive,s:0.8}（这包括小丑牌及能量牌）{}",
					"{C:inactive}（在给予{C:chips}#2#/400{}{C:inactive}筹码后进化）"
				}
			},
			j_poke_muk = {
				name = "臭臭泥",
				text = {
					"在你的牌组中",
					"每张多于{C:attention}#3#{}张牌都获得{C:mult}+#1#{}倍数",
					"如果你的牌组有多于{C:attention}#4#{}张牌，{X:mult,C:white} X#2# {}倍数",
					"{C:inactive,s:0.8}（目前为{C:mult,s:0.8}+#5#{}{C:inactive,s:0.8}倍数）{}",
					"每回合结束时",
					"增加两张随机的扑克牌到牌组",
					"并从牌组中移除一张随机的扑克牌"
				}
			},
			j_poke_munchlax = {
				name = "小卡比兽",
				text = {
					"{C:attention}婴儿{}，{X:red,C:white} X#1# {}倍数",
					"回合结束后，产生一张随机的",
					"{C:dark_edition}负片{}{C:item}物品牌{}",
					"{C:inactive}（你没看错，这会{C:attention}减低{C:inactive}你的倍数）",
					"{C:inactive}（在{C:attention}#2#{}{C:inactive}个回合后进化）"
				}
			},
			j_poke_nidoking = {
				name = "尼多王",
				text = {
					"{C:attention}+#2#{}手牌数量",
					"手牌中，每张{C:attention}K{}",
					"都给予{C:mult}+#1#{}倍数"
				}
			},
			j_poke_nidoqueen = {
				name = "尼多后",
				text = {
					"{C:attention}+#2#{}手牌数量",
					"手牌中，每张{C:attention}Q{}",
					"都给予{C:chips}+#1#{}筹码"
				}
			},
			j_poke_nidoranf = {
				name = "尼多兰",
				text = {
					"手牌中，每张{C:attention}Q{}",
					"都给予{C:chips}+#1#{}筹码",
					"{C:inactive}（在{C:attention}#2#{}{C:inactive}个回合后进化）"
				}
			},
			j_poke_nidoranm = {
				name = "尼多朗",
				text = {
					"手牌中，每张{C:attention}K{}",
					"都给予{C:mult}+#1#{}倍数",
					"{C:inactive}（在{C:attention}#2#{}{C:inactive}个回合后进化）"
				}
			},
			j_poke_nidorina = {
				name = "尼多娜",
				text = {
					"手牌中，每张{C:attention}Q{}",
					"都给予{C:chips}+#1#{}筹码",
					"{C:inactive}（使用{}{C:attention}月之石{}{C:inactive}牌进化）"
				}
			},
			j_poke_nidorino = {
				name = "尼多力诺",
				text = {
					"手牌中，每张{C:attention}K{}",
					"都给予{C:mult}+#1#{}倍数",
					"{C:inactive}（使用{}{C:attention}月之石{}{C:inactive}牌进化）"
				}
			},
			j_poke_ninetales = {
				name = "九尾",
				text = {
					"{C:attention}在持有{}{C:spectral}灵媒{}{C:attention}牌时",
					"每张计分的{C:attention}9{}",
					"有{C:green}#1#/#2#{}的几率",
					"产生一张{C:attention}塔罗{}或{C:planet}行星{}牌",
					"{C:inactive}（必须有空位）"
				}
			},
			j_poke_oddish = {
				name = "走路草",
				text = {
					"每张计分的{C:attention}奇数{}点的牌",
					"都给予{C:mult}+#1#{}或{C:mult}+#2#{}倍数",
					"{C:inactive}（在{C:attention}#3#{}{C:inactive}个回合后进化）"
				}
			},
			j_poke_omanyte = {
				name = "菊石兽",
				text = {
					"{C:attention}原始的#1#{}",
					"{C:attention}一张{}：此牌的出售价值{}提升{C:money}$#2#{}",
					"{C:attention}两张{}：获得{C:money}$#3#{}",
					"{C:attention}三张或以上{}：产生一张随机的{C:attention}塔罗牌{}{C:inactive}（必须有空位）",
					"{C:inactive}（在{C:attention}#4#{}{C:inactive}个回合后进化）"
				}
			},
			j_poke_omastar = {
				name = "多刺菊石兽",
				text = {
					"{C:attention}原始的#1#{}",
					"{C:attention}一张{}：此牌的出售价值{}提升{C:money}$#2#{}",
					"{C:attention}两张{}：获得{C:money}$#3#{}",
					"{C:attention}三张{}：产生一张随机的{C:attention}塔罗牌{}",
					"{C:attention}四张或以上{}：产生一张随机的{C:item}物品牌{}",
					"{C:inactive}（必须有空位）"
				}
			},
			j_poke_onix = {
				name = "大岩蛇",
				text = {
					"{C:attention}第一次出牌{}时，最左边的计分牌",
					"会变成{C:attention}石头牌{}",
					"{C:inactive}（附上{C:metal}钢{}{C:inactive}贴纸后进化）{}"
				}
			},
			j_poke_paras = {
				name = "派拉斯",
				text = {
					"如果打出的牌包含{C:attention}两对{}",
					"此小丑牌获得{C:mult}+#2#{}倍数",
					"{C:inactive}（目前为{C:mult}+#1#{}{C:inactive}倍数）",
					"{C:inactive}（在{C:mult}+8{}{C:inactive}倍数时进化)"
				}
			},
			j_poke_parasect = {
				name = "派拉斯特",
				text = {
					"如果打出的牌包含{C:attention}两对{}",
					"此小丑牌获得{C:mult}+#2#{}倍数",
					"否则{C:mult}-#3#{}倍数",
					"{C:inactive}（目前为{C:mult}+#1#{}{C:inactive}倍数）"
				}
			},
			j_poke_persian = {
				name = "猫老大",
				text = {
					"每回合结束时",
					"获得最左边小丑牌卖出价格的",
					"{C:attention}双倍{}{C:money}金钱{}",
					"{C:inactive}（自身除外）{}",
					"{C:inactive}（目前为{C:money}$#1#{C:inactive}，最多{C:money}$15{C:inactive}）"
				}
			},
			j_poke_pichu = {
				name = "皮丘",
				text = {
					"{C:attention}婴儿{}，{X:red,C:white} X#2# {}倍数",
					"回合结束时",
					"获得{C:money}$#1#{}",
					"{C:inactive}（在{C:attention}#3#{}{C:inactive}个回合后进化）"
				}
			},
			j_poke_pidgeot = {
				name = "大比鸟",
				text = {
					"商店里的{C:planet}行星牌{}以及",
					"{C:planet}天外礼包{}全部免费",
					"使用{C:planet}行星牌{}时获得{C:money}$#1#{}"
				}
			},
			j_poke_pidgeotto = {
				name = "比比鸟",
				text = {
					"商店里的{C:planet}行星牌{}以及",
					"{C:planet}天外礼包{}的费用",
					"减低{C:money}$3{}",
					"{C:inactive}（在{C:attention}#1#{}{C:inactive}个回合后进化）"
				}
			},
			j_poke_pidgey = {
				name = "波波",
				text = {
					"商店里的{C:planet}行星牌{}以及",
					"{C:planet}天外礼包{}的费用",
					"减低{C:money}$2{}",
					"{C:inactive}（在{C:attention}#1#{}{C:inactive}个回合后进化）"
				}
			},
			j_poke_pikachu = {
				name = "皮卡丘",
				text = {
					"当回合结束时，每一张小丑牌",
					"获得{C:money}$#1#{}",
					"{C:inactive}（最多{C:money}$10{C:inactive}）",
					"{C:inactive}（使用{}{C:attention}雷之石{}{C:inactive}牌进化）"
				}
			},
			j_poke_pinsir = {
				name = "凯罗斯",
				text = {
					"如果{C:attention}最左边{}打出的牌",
					"和{C:attention}最右边{}打出的牌的{C:attention}点数{}相同",
					"倍数{X:mult,C:white} X#1# {}"
				}
			},
			j_poke_pokedex = {
				name = "宝可梦图鉴",
				text = {
					"每个有{C:pink}属性{}的小丑牌",
					"会给予{C:mult}+#2#{}倍数",
					"{C:attention}宝可梦{}小丑牌可能会重复出现",
					"{C:inactive}（目前为{C:mult}+#1#{C:inactive}倍数）"
				}
			},
			j_poke_politoed = {
				name = "蚊香蛙皇",
				text = {
					"每张计分的{V:1}#2#{}花色的牌",
					"会给予{C:mult}+#1#{}倍数",
					"那些牌会依{X:water,C:white}水属性{}",
					"的小丑牌的数量而重新触发",
					"{C:inactive,s:0.8}（{C:attention,s:0.8}#7#{}{C:inactive,s:0.8}次的重新触发数量会平均分配到每张的计分牌）{}",
					"花色在出牌后按次序地变更{C:inactive,s:0.8}(#3#, #4#, #5#, #6#){}"
				}
			},
			j_poke_poliwag = {
				name = "蚊香蝌蚪",
				text = {
					"每张计分的{V:1}#3#{}花色的牌",
					"给予{C:mult}+#1#{}倍数",
					"花色在出牌后按次序地变更",
					"{C:inactive,s:0.8}(#4#, #5#, #6#, #7#){}",
					"{C:inactive}（在{C:attention}#2#{}{C:inactive}个回合后进化）"
				}
			},
			j_poke_poliwhirl = {
				name = "蚊香君",
				text = {
					"每张计分的{V:1}#2#{}花色的牌",
					"给予{C:mult}+#1#{}倍数",
					"花色在出牌后按次序地变更",
					"{C:inactive,s:0.8}(#3#, #4#, #5#, #6#){}",
					"{C:inactive,s:0.8}（使用{}{C:attention,s:0.8}水之石{}{C:inactive,s:0.8}或{C:attention,s:0.8}王者之证{}{C:inactive,s:0.8}牌进化）"
				}
			},
			j_poke_poliwrath = {
				name = "蚊香泳士",
				text = {
					"每张计分的{V:1}#2#{}花色的牌",
					"给予{C:mult}+#7#{}倍数及{X:mult,C:white} X#1# {}倍数",
					"花色在出牌后按次序地变更",
					"{C:inactive,s:0.8}(#3#, #4#, #5#, #6#){}"
				}
			},
			j_poke_ponyta = {
				name = "小火马",
				text = {
					"如果打出的牌包含{C:attention}顺子{}",
					"此小丑牌会获得{C:chips}#2#{}筹码",
					"{C:inactive}（目前为{C:chips}+#1#{C:inactive}筹码）",
					"{C:inactive}（在{C:chips}+60{}{C:inactive}筹码时进化）"
				}
			},
			j_poke_porygon = {
				name = "多边兽",
				text = {
					"{C:pink}+1{}能量上限",
					"每次打开{C:attention}扩充包{}时",
					"产生一张{C:pink}能量{}牌",
					"{C:inactive}（使用{} {C:attention}升级资料{}{C:inactive}牌进化）"
				}
			},
			j_poke_porygon2 = {
				name = "多边兽Ⅱ",
				text = {
					"{C:pink}+2{}能量上限",
					"每次打开{C:attention}扩充包{}时",
					"产生一张与最左边的小丑牌的{C:pink}属性{}",
					"相同的{C:pink}能量{}牌",
					"{C:inactive}（使用{}{C:attention}可疑修正档{}{C:inactive}牌进化）"
				}
			},
			j_poke_porygonz = {
				name = "多边兽Ｚ",
				text = {
					"{C:pink}+3{}能量上限",
					"每张在此{C:attention}比赛{}中使用的{C:pink}能量{}牌会给予{X:red,C:white} X#2# {}倍数",
					"{C:inactive}（目前为{X:red,C:white} X#1# {}{C:inactive}倍数）"
				}
			},
			j_poke_primeape = {
				name = "火爆猴",
				text = {
					"每张计分的{C:attention}2{},",
					"{C:attention}3{}或{C:attention}5{}给予",
					"{C:mult}+#1#{}倍数及{C:chips}+#2#{}筹码",
					"{C:inactive}（在触发{C:attention}#3#{}{C:inactive}/25 次后进化）{}"
				}
			},
			j_poke_psyduck = {
				name = "可达鸭",
				text = {
					"如果打出的牌为",
					"单张{C:attention}人头牌{}，获得{C:money}$#1#{}",
					"{C:inactive}（在{C:attention}#2#{}{C:inactive}个回合后进化）"
				}
			},
			j_poke_raichu = {
				name = "雷丘",
				text = {
					"如果拥有超过{C:money}$#2#{}，在回合结束时",
					"此牌会新增{C:dark_edition}负片{}效果",
					"{C:inactive,s:0.8}（需要的金钱会依雷丘的数量而增加）{}",
					"当回合结束时，每一张小丑牌",
					"获得{C:money}$#1#{}",
					"{C:inactive}（最多{C:money}$10{C:inactive}）"
				}
			},
			j_poke_rapidash = {
				name = "烈焰马",
				text = {
					"如果打出的牌包含{C:attention}顺子{}",
					"此小丑牌会获得{C:chips}#2#{}筹码",
					"第一次出牌时附上{C:attention}捷径{}效果",
					"{C:inactive}（目前为{C:chips}+#1#{C:inactive}筹码）"
				}
			},
			j_poke_raticate = {
				name = "拉达",
				text = {
					"重新触发得分中",
					"{C:attention}头两张{}打出的牌",
					"额外{C:attention}#1#{}次"
				}
			},
			j_poke_rattata = {
				name = "小拉达",
				text = {
					"重新触发得分中",
					"{C:attention}最先{}打出的牌",
					"额外{C:attention}#1#{}次",
					"{C:inactive}（在{C:attention}#2#{}{C:inactive}个回合后进化）"
				}
			},
			j_poke_rhydon = {
				name = "钻角犀兽",
				text = {
					"每张计分的{C:attention}石头牌{}",
					"可永久获得{C:chips}+#1#{}筹码",
					"{C:inactive}（使用{}{C:attention}联系绳{}{C:inactive}牌进化）"
				}
			},
			j_poke_rhyhorn = {
				name = "独角犀牛",
				text = {
					"每张计分的{C:attention}石头牌{}",
					"可永久获得{C:chips}+#1#{}筹码",
					"{C:inactive}（在{C:attention}#2#{}{C:inactive}个回合后进化）"
				}
			},
			j_poke_rhyperior = {
				name = "超甲狂犀",
				text = {
					"每张计分的{C:attention}石头牌{}",
					"可永久获得{C:chips}+#1#{}筹码",
					"每张{X:earth,C:white}地{}属性小丑牌会重新触发{C:attention}石头牌{}",
					"{C:inactive}（目前有#2#次重新触发）"
				}
			},
			j_poke_sandshrew = {
				name = "穿山鼠",
				text = {
					"当{C:attention}玻璃牌{}被摧毁时",
					"增加一张复制的{C:attention}石头牌{}",
					"到牌组并抽到{C:attention}手牌{}中",
					"{C:inactive}（在{C:attention}#1#{}{C:inactive}个回合后进化）"
				}
			},
			j_poke_sandslash = {
				name = "穿山王",
				text = {
					"当{C:attention}玻璃牌{}被摧毁时",
					"增加一张复制的{C:attention}钢铁牌{}",
					"到牌组并抽到{C:attention}手牌{}中"
				}
			},
			j_poke_sceptile = {
				name = "蜥蜴王",
				text = {
					"{C:attention}+#3#{}手牌数量，{C:attention}多种性格{}",
					"打出的{C:attention}#5#，#6#或#7#{}牌会获得{C:money}$#1#{}",
					"回合结束后，每一张{X:grass,C:white}草{}属性牌会给予{C:money}$#1#{}",
					"{C:inactive,s:0.8}（这包括小丑牌及能量牌）{}",
					"{C:inactive}（目前为{C:money}$#4#{}，最多{C:money}$14{}{C:inactive}）{}"
				}
			},
			j_poke_scizor = {
				name = "巨钳螳螂",
				text = {
					"选择盲注后，摧毁右边的小丑牌",
					"如果摧毁的小丑牌不是{C:attention}常见{}",
					"这张牌会获得{C:attention}银箔{}，{C:attention}全像摄影{}或{C:attention}彩色{}",
					"那些版本会在此牌{C:attention}叠加{}",
					"{C:inactive,s:0.75}（会优先选择被摧毁的小丑牌如有的版本）{}",
					"{C:inactive}（目前为{C:mult}+#1#{}{C:inactive}倍数，{C:chips}+#2#{}{C:inactive}筹码，{X:red,C:white}X#3#{} {C:inactive}倍数)"
				}
			},
			j_poke_scyther = {
				name = "飞天螳螂",
				text = {
					"选择盲注后，摧毁右边的小丑牌并获得{C:mult}+#2#{}倍数",
					"如果被摧毁的小丑牌是{C:red}稀有{}或以上",
					"新增{C:attention}银箔{}，{C:attention}全像摄影{}或{C:attention}彩色{}版本",
					"{C:inactive}（附上{C:metal}钢{}{C:inactive}贴纸后进化）{}",
					"{C:inactive}（目前为{C:mult}+#1#{C:inactive} 倍数）"
				}
			},
			j_poke_seadra = {
				name = "海刺龙",
				text = {
					"在{C:attention}头两次出牌{}中，每张计分的{C:attention}6{}",
					"会给此牌{C:mult}+#2#{}倍数",
					"{C:inactive}（目前为{C:mult}+#1#{C:inactive}倍数）",
					"{C:inactive}（附上{C:dragon}龙{}{C:inactive}贴纸后进化）{}"
				}
			},
			j_poke_seaking = {
				name = "金鱼王",
				text = {
					"重新触发在手牌中所有的",
					"{C:attention}黄金牌{}{C:attention}#1#{}次"
				}
			},
			j_poke_seel = {
				name = "小海狮",
				text = {
					"在{C:attention}第一次{}出牌中",
					"有{C:green}#1#/#2#{}的几率",
					"新增随机的{C:attention}封蜡章{}到最左边的计分牌",
					"{C:inactive,s:0.75}（在牌组中拥有至少{C:attention,s:0.75}#3#{}{C:inactive,s:0.75}{C:attention,s:0.75}个封蜡章{C:inactive,s:0.75}时进化）"
				}
			},
			j_poke_sentret = {
				name = "尾立",
				text = {
					"如果打出的牌型{C:attention}连续{}不是上一个的牌型",
					"这张牌会获得{C:mult}+#2#{}倍数",
					"{C:inactive}（上一个牌型：{C:attention}#3#{}{C:inactive}）",
					"{C:inactive}（目前为{C:mult}+#1#{} {C:inactive}倍数）",
					"{C:inactive}（在{C:mult}+15{} {C:inactive}倍数时进化）"
				}
			},
			j_poke_shellder = {
				name = "大舌贝",
				text = {
					"如果打出的牌有计分的{C:attention}5{}",
					"每一张有{C:green}#1#/#2#{}的几率重新触发",
					"{C:inactive}（使用{}{C:attention}水之石{}{C:inactive}牌进化）"
				}
			},
			j_poke_slowbro = {
				name = "呆壳兽",
				text = {
					"每次出牌获得{X:red,C:white} X#1# {}倍数",
					"回合结束时重设",
					"{C:inactive}（目前为{X:red,C:white} X#2# {}{C:inactive}倍数）"
				}
			},
			j_poke_slowking = {
				name = "呆呆王",
				text = {
					"每张计分的{C:attention}K{}会给予{X:red,C:white}X#1#{}倍数",
					"每次出牌后增加{X:red,C:white}X#2#{}倍数",
					"回合结束后重设"
				}
			},
			j_poke_slowpoke = {
				name = "呆呆兽",
				text = {
					"每回合的{C:attention}最后一次出牌{}",
					"可得到{X:red,C:white} X#1# {}倍数",
					"{C:inactive}（在{C:attention}#2#{}{C:inactive}个回合后进化）"
				}
			},
			j_poke_slowpoke2 = {
				name = "呆呆兽",
				text = {
					"每回合的{C:attention}最后一次出牌{}可得到{X:red,C:white} X#1# {}倍数",
					"每回合结束后，有{C:green}#3#/#4#{}几率",
					"产生一张{C:attention}王者之证{}牌{C:inactive,s:0.8}（必须有空位）{}",
					"{C:inactive,s:0.8}（在{C:attention,s:0.8}#2#{}{C:inactive,s:0.8}个回合后或使用{C:attention,s:0.8}王者之证{}{C:inactive,s:0.8}牌进化）"
				}
			},
			j_poke_smoochum = {
				name = "迷唇娃",
				text = {
					"{C:attention}婴儿{}，{X:red,C:white} X#1# {}倍数",
					"当这个小丑牌进化时",
					"产生一个{C:attention}标准{}标签",
					"{C:inactive}（你没看错，这会{C:attention}减低{C:inactive}你的倍数）",
					"{C:inactive}（在{C:attention}#2#{}{C:inactive}个回合后进化）"
				}
			},
			j_poke_snorlax = {
				name = "卡比兽",
				text = {
					"当你得到这张牌后",
					"产生一张{C:attention}吃剩的东西{}牌",
					"回合结束时，每张{C:attention}吃剩的东西{}牌",
					"会给此牌{X:red,C:white}X#1#{}倍数",
					"{C:inactive}（目前为{X:red,C:white} X#2# {}{C:inactive}倍数）"
				}
			},
			j_poke_spearow = {
				name = "烈雀",
				text = {
					"当有{C:attention}#2#{}张牌计分后",
					"升级下一个打出的{C:attention}牌型{}",
					"{C:inactive}（目前有{C:attention}#3#{}{C:inactive}/#2#）{C:inactive}#4#{}",
					"{C:inactive}（在{C:attention}#1#{}{C:inactive}个回合后进化）"
				}
			},
			j_poke_squirtle = {
				name = "杰尼龟",
				text = {
					"{C:chips}+#3#{}出牌次数",
					"当回合结束时，每一个未使用的",
					"出牌次数会给此牌{C:chips}+#2#{}筹码",
					"{C:inactive}（目前为{C:chips}+#1#{C:inactive}筹码）",
					"{C:inactive}（在{C:chips}+16{}{C:inactive}筹码时进化）"
				}
			},
			j_poke_stantler = {
				name = "惊角鹿",
				text = {
					"如果打出的手牌包含{C:attention}对子{}",
					"给予{C:chips}+#1#{}筹码乘以",
					"第一张记分牌的{C:attention}默认{}筹码",
					"在{C:attention}最后一次{}出牌时，给予的筹码会{C:attention}加倍{}"
				}
			},
			j_poke_starmie = {
				name = "宝石海星",
				text = {
					"每张计分的{C:diamonds}#2#{}花色的牌",
					"给予{C:mult}+#1#{}倍数及{C:money}$#2#{}"
				}
			},
			j_poke_staryu = {
				name = "海星星",
				text = {
					"每张计分的{C:diamonds}#2#{}花色的牌",
					"给予{C:mult}+#1#{}倍数",
					"{C:inactive}（使用{}{C:attention}水之石{}{C:inactive}牌进化）"
				}
			},
			j_poke_steelix = {
				name = "大钢蛇",
				text = {
					"{C:attention}第一次出牌{}时，最左边的计分牌",
					"会变成{C:attention}钢铁牌{}",
					"在{C:attention}手中{}的{C:attention}石头牌{}会变成{C:attention}钢铁牌{}"
				}
			},
			j_poke_swampert = {
				name = "巨沼怪",
				text = {
					"{C:chips}+#3#{}出牌次数，{C:attention}多种性格{}",
					"打出的{C:attention}#6#, #7# or #8#{}会给予{C:chips}+#1#{}筹码",
					"如果你有其他的{X:water,C:white}水{}属性或{X:earth,C:white}地{}属性的牌，每张额外给予{C:chips}+#5#{}筹码",
					"{C:inactive,s:0.8}（这包括小丑牌及能量牌）{}",
					"{C:inactive}（目前给予了{C:chips}+#4#{}{C:inactive}筹码）"
				}
			},
			j_poke_sylveon = {
				name = "仙子伊布",
				text = {
					"每{C:attention}三{}次的{C:green}重铩{}",
					"会产生一张{C:attention}星星{}牌",
					"每张在手中的{C:diamonds}#2#{}花色的牌",
					"会给予{X:mult,C:white} X#3# {}倍数",
					"{C:inactive}（必须有空位）",
					"{C:inactive}（目前{C:attention}#1#{}{C:inactive}/3次重铩）"
				}
			},
			j_poke_tall_grass = {
				name = "草丛",
				text = {
					"出牌时，有{C:green}#1#/#2#{}的几率",
					"产生一张{C:chips}一般{}的宝可梦{C:attention}小丑牌{}",
					"如果计分牌中包含{C:attention}万能牌{}，效果会确保触发"
				}
			},
			j_poke_tangela = {
				name = "蔓藤怪",
				text = {
					"每张计分的{C:attention}万能牌{}",
					"给予{C:mult}+#1#{}倍数，{C:chips}+#2#{}筹码或{C:money}$#3#{}",
					"有{C:green}#4#/#5#{}的几率给予{C:attention}以上三个的加成{}",
					"{C:inactive}（在牌组有20%或以上是{C:attention}万能牌{C:inactive}时进化）"
				}
			},
			j_poke_tangrowth = {
				name = "巨蔓藤",
				text = {
					"每张计分的{C:attention}万能牌{}会给予",
					"{C:mult}+#1#{}倍数，{C:chips}+#2#{}筹码或{C:money}$#3#{}",
					"有{C:green}#4#/#5#{}的几率给予{C:attention}以上三个的加成{}",
					"万能牌{C:attention}不能{}被减益"
				}
			},
			j_poke_tauros = {
				name = "肯泰罗（首领）",
				text = {
					"每一张{C:attention}肯泰罗{}小丑牌给予{X:mult,C:white} X#1#{}倍数",
					"每次在商店中重铩会有{C:green}#2#/#3#{}的几率",
					"新增一张{C:attention}肯泰罗（牛群）{}到商店中"
				}
			},
			j_poke_taurosh = {
				name = "肯泰罗（牛群）",
				text = {
					"{C:mult}+#1#{}倍数"
				}
			},
			j_poke_tentacool = {
				name = "玛瑙水母",
				text = {
					"如果打出的牌只有{C:attention}10{}",
					"每张计分的{C:attention}10{}",
					"给予{C:mult}+#1#{}倍数",
					"{C:inactive}（在{C:attention}#2#{}{C:inactive}个回合后进化）"
				}
			},
			j_poke_tentacruel = {
				name = "毒刺水母",
				text = {
					"每张计分的{C:attention}10{}",
					"给予{C:mult}+#1#{}倍数",
					"{C:attention}10牌不能{}被减益"
				}
			},
			j_poke_tinkatink = {
				name = "小锻匠",
				text = {
					"每张打出的牌给予{C:mult}+#1#{}倍数",
					"选择盲注后，减益{C:attention}#3#{}张在牌组里",
					"不是{C:attention}钢铁牌{}的牌",
					"{C:inactive}（在{C:attention}#2#{C:inactive}个回合后进化）"
				}
			},
			j_poke_tinkaton = {
				name = "巨锻匠",
				text = {
					"每张打出的牌给予{C:mult}+#1#{}倍数",
					"选择盲注后，减益{C:attention}#3#{}张在牌组里",
					"不是{C:attention}钢铁牌{}的牌",
					"打出的{C:attention}钢铁牌{}会有{C:attention}玻璃牌{}的效果"
				}
			},
			j_poke_tinkatuff = {
				name = "巧锻匠",
				text = {
					"每张打出的牌给予{C:mult}+#1#{}倍数",
					"选择盲注后，减益{C:attention}#3#{}张在牌组里",
					"不是{C:attention}钢铁牌{}的牌",
					"{C:inactive}（在{C:attention}#2#{C:inactive}个回合后进化）"
				}
			},
			j_poke_torchic = {
				name = "火稚鸡",
				text = {
					"{C:mult}+#3#{}弃牌次数，{C:attention}多种性格{}",
					"这回合中，每张弃掉的{C:attention}#5#，#6#或#7#{}会给予{C:mult}+#1#{}倍数",
					"如果你有其他的{X:fire,C:white}火{}属性或{X:earth,C:white}斗{}属性的牌，给予的倍数翻倍",
					"{C:inactive,s:0.8}（这包括小丑牌及能量牌）{}",
					"{C:inactive}（目前为{C:mult}#4#{}{C:inactive}倍数）{}",
					"{C:inactive}（在给予{C:mult}#2#/60{}{C:inactive}倍数后进化）"
				}
			},
			j_poke_treecko = {
				name = "木守宫",
				text = {
					"{C:attention}+#3#{}手牌数量，{C:attention}多种性格{}",
					"打出的{C:attention}#6#，#7#或#8#{}牌",
					"会有{C:green}#4#/#5#{}的几率获得{C:money}$#1#{}",
					"如果你有其他的{X:grass,C:white}草{}属性牌，效果确保触发",
					"{C:inactive,s:0.8}（这包括小丑牌及能量牌）{}",
					"{C:inactive}（在赚取{C:money}$#2#/16{}后进化）"
				}
			},
			j_poke_tyrogue = {
				name = "无畏小子",
				text = {
					"{C:attention}婴儿{}，{X:red,C:white} X#1# {}倍数",
					"在回合的{C:attention}第一次出牌或弃牌{}中",
					"如果选择了{C:attention}五{}张牌，有随机一张牌",
					"会复制如果是出牌，或摧毁如果是弃牌",
					"{C:inactive}（在{C:attention}#2#{}{C:inactive}个回合后进化）"
				}
			},
			j_poke_umbreon = {
				name = "月亮伊布",
				text = {
					"每{C:attention}三{}次的{C:green}重铩{}",
					"会产生一张{C:attention}月亮{}牌",
					"在{C:attention}最后一次{}出牌时，重新触发",
					"所有{C:clubs}#2#{}花色牌的",
					"{C:attention}持在手中{}的效果",
					"{C:inactive}（必须有空位）",
					"{C:inactive}（目前{C:attention}#1#{}{C:inactive}/3次重铩）"
				}
			},
			j_poke_vaporeon = {
				name = "水伊布",
				text = {
					"每{C:attention}三{}次的{C:green}重铩{}",
					"会给此牌{C:chips}+#2#{}筹码",
					"{C:inactive}（目前有{C:attention}#3#{}{C:inactive}/3次重铩）",
					"{C:inactive}（目前为{C:chips}+#1#{C:inactive}筹码）"
				}
			},
			j_poke_venomoth = {
				name = "摩鲁蛾",
				text = {
					"对所有{C:attention}几率牌{}的{C:green,E:1,S:1.1}几率{}加{C:attention}#1#{}",
					"{C:inactive}（例：{C:green}1/6{C:inactive} -> {C:green}3/6{C:inactive}）"
				}
			},
			j_poke_venonat = {
				name = "毛球",
				text = {
					"对所有{C:attention}几率牌{}的{C:green,E:1,S:1.1}几率{}加{C:attention}#1#{}",
					"{C:inactive}（例：{C:green}1/6{C:inactive} -> {C:green}2/6{C:inactive}）",
					"{C:inactive}（在{C:attention}#2#{}{C:inactive}个回合后进化）"
				}
			},
			j_poke_venusaur = {
				name = "妙蛙花",
				text = {
					"{C:attention}+#3#{}手牌数量",
					"手牌中，每一张{C:attention}#4#{}会给予{C:money}$#1#{}",
					"卡牌点数于每回合变更",
					"{C:inactive}（目前已获得{C:money}$#2#{C:inactive}）"
				}
			},
			j_poke_victreebel = {
				name = "大食花",
				text = {
					"每张计分的{C:attention}偶数{}点的牌",
					"给予{C:chips}+#1#{}筹码",
					"并{C:attention}重新触发{}"
				}
			},
			j_poke_vikavolt = {
				name = "锹农炮虫",
				text = {
					"{C:mult}+#3#{}倍数",
					"每张其他你拥有的{X:lightning, C:black}雷{}属性小丑牌",
					"会给予{X:red,C:white} X#1# {}倍数",
					"{C:inactive}（目前为{X:red,C:white} X#2# {}{C:inactive}倍数）"
				}
			},
			j_poke_vileplume = {
				name = "霸王花",
				text = {
					"每张计分的{C:attention}奇数{}点的牌",
					"都给予{C:mult}+#2#{}倍数或{X:mult,C:white} X#1# {}倍数"
				}
			},
			j_poke_voltorb = {
				name = "霹雳电球",
				text = {
					"如果这张牌是{C:attention}最左边{}的小丑牌",
					"给予{C:mult}+#1#{}倍数并减益这张小丑牌",
					"直到回合结束",
					"{C:inactive}（在{C:attention}#2#{}{C:inactive}个回合后进化）"
				}
			},
			j_poke_vulpix = {
				name = "六尾",
				text = {
					"每张计分的{C:attention}9{}",
					"有{C:green}#1#/#2#{}的几率",
					"产生一张{C:attention}塔罗{}牌",
					"{C:inactive}（必须有空位）",
					"{C:inactive}（使用{}{C:attention}火之石{}{C:inactive}牌进化）"
				}
			},
			j_poke_wartortle = {
				name = "卡咪龟",
				text = {
					"{C:chips}+#3#{}出牌次数",
					"当回合结束时，每一个未使用的",
					"出牌次数会给此牌{C:chips}+#2#{}筹码",
					"{C:inactive}（目前为{C:chips}+#1#{C:inactive}筹码）",
					"{C:inactive}（在{C:chips}+36{}{C:inactive}筹码时进化）"
				}
			},
			j_poke_weedle = {
				name = "独角虫",
				text = {
					"{C:chips}+#1#{}筹码",
					"{C:inactive}（在{C:attention}#2#{}{C:inactive}个回合后进化）"
				}
			},
			j_poke_weepinbell = {
				name = "口呆花",
				text = {
					"每张计分的{C:attention}偶数{}点的牌",
					"给予{C:chips}+#1#{}筹码",
					"{C:inactive}（使用{}{C:attention}叶之石{}{C:inactive}牌进化）"
				}
			},
			j_poke_weezing = {
				name = "双弹瓦斯",
				text = {
					"卖出这张牌来减低",
					"目前{C:attention}Boss 盲注{}所需的一半分数",
					"并消除限制效果"
				}
			},
			j_poke_wigglytuff = {
				name = "胖可丁",
				text = {
					"每张计分的{C:spades}#3#{}花色的牌",
					"都给予{C:mult}+#1#{}倍数，{C:chips}+#2#{}筹码",
					"及卡牌给予的筹码"
				}
			},
			j_poke_yamper = {
				name = "来电汪",
				text = {
					"如果打出的牌包含{C:attention}顺子{}",
					"给予{C:mult}+#1#{}倍数及获得{C:money}$#2#{}",
					"{C:inactive}（在{C:attention}#3#{}{C:inactive}个回合后进化）"
				}
			},
			j_poke_zapdos = {
				name = "闪电鸟",
				text = {
					"每持有的{C:money}$#2#{}",
					"会给予{X:mult,C:white} X#1# {}倍数",
					"{C:inactive}（目前为{X:red,C:white}X#3#{}{C:inactive}倍数）"
				}
			},
			j_poke_zubat = {
				name = "超音蝠",
				text = {
					"每张在牌组中的加强牌",
					"都给予{C:mult}+#1#{}倍数",
					"{C:inactive}（目前为{C:mult}+#2#{}{C:inactive}倍数，{C:attention}#4#{}{C:inactive}加强牌）",
					"{C:inactive}（在拥有{C:mult}#3#{C:inactive}张加强牌时进化）"
				}
			}
		},
		Other = {
			Bird = {
				name = "属性",
				text = {
					"{X:bird,C:white}鸟{}"
				}
			},
			Colorless = {
				name = "属性",
				text = {
					"{X:colorless,C:white}一般{}"
				}
			},
			Dark = {
				name = "属性",
				text = {
					"{X:dark,C:white}恶{}"
				}
			},
			Dragon = {
				name = "属性",
				text = {
					"{X:dragon,C:white}龙{}"
				}
			},
			Earth = {
				name = "属性",
				text = {
					"{X:earth,C:white}地面{}"
				}
			},
			Fairy = {
				name = "属性",
				text = {
					"{X:fairy,C:white}妖精{}"
				}
			},
			Fighting = {
				name = "属性",
				text = {
					"{X:fighting,C:white}格斗{}"
				}
			},
			Fire = {
				name = "属性",
				text = {
					"{X:fire,C:white}火{}"
				}
			},
			Grass = {
				name = "属性",
				text = {
					"{X:grass,C:white}草{}"
				}
			},
			Lightning = {
				name = "属性",
				text = {
					"{X:lightning,C:black}电{}"
				}
			},
			Metal = {
				name = "属性",
				text = {
					"{X:metal,C:white}钢{}"
				}
			},
			Psychic = {
				name = "属性",
				text = {
					"{X:psychic,C:white}超能力{}"
				}
			},
			Water = {
				name = "属性",
				text = {
					"{X:water,C:white}水{}"
				}
			},
			ancient = {
				name = "原始之力",
				text = {
					"这张牌的效果",
					"会以打出的{C:attention}#1#{}的数量来决定",
					"{C:inactive}（效果会{C:attention}累计地触发{}{C:inactive}）"
				}
			},
			availability = {
				name = "现身条件",
				text = {
					"如果你有{C:attention}非保久的小丑牌{}",
					"这张牌就不会出现"
				}
			},
			baby = {
				name = "婴儿",
				text = {
					"出牌时，如果有{C:attention}非婴儿{}",
					"小丑牌在这张牌的右边",
					"这张牌会被{C:attention}减益{}"
				}
			},
			basic = {
				name = "基础",
				text = {
					"未曾{C:attention}进化{}的",
					"宝可梦小丑牌"
				}
			},
			chip_progress = {
				name = "获得的筹码进度",
				text = {
					"对获得的{C:chip}筹码{}",
					"的{C:attention}#1#%{}进度增加{C:chip}+1{}"
				}
			},
			colorless_sticker = {
				name = "属性",
				text = {
					"{X:colorless,C:white}一般{}"
				}
			},
			dark_sticker = {
				name = "属性",
				text = {
					"{X:dark,C:white}恶{}"
				}
			},
			deli_gift = {
				name = "礼物",
				text = {
					"{C:green}35%{} - {C:money}$8{}",
					"{C:green}30%{} - {C:item}物品{}{C:attention}牌",
					"{C:green}20%{} - {C:attention}优惠券标签",
					"{C:green}15%{} - {C:dark_edition}彩色{}{C:attention}礼物卡"
				}
			},
			dragon_sticker = {
				name = "属性",
				text = {
					"{X:dragon,C:white}龙{}"
				}
			},
			earth_sticker = {
				name = "属性",
				text = {
					"{X:earth,C:white}地面{}"
				}
			},
			eeveelution = {
				name = "伊布家族",
				text = {
					"{C:attention}水之石{} - {X:water,C:white}水伊布{}",
					"{C:attention}雷之石{} - {X:lightning,C:black}雷伊布{}",
					"{C:attention}火之石{} - {X:fire,C:white}火伊布{}",
					"{C:attention}日之石{} - {X:psychic,C:white}太阳伊布{}",
					"{C:attention}月之石{} - {X:dark,C:white}月亮伊布{}",
					"{C:attention}叶之石{} - {X:grass,C:white}叶伊布{}",
					"{C:attention}冰之石{} - {X:water,C:white}冰伊布{}",
					"{C:attention}光之石{} - {X:fairy,C:white}仙子伊布{}"
				}
			},
			eitem = {
				name = "进化牌",
				text = {
					"对最左边及{C:attention}适合的{}",
					"小丑牌进行进化"
				}
			},
			energy = {
				name = "能量",
				text = {
					"{C:attention}#1#{}/#2#"
				}
			},
			fairy_sticker = {
				name = "属性",
				text = {
					"{X:fairy,C:white}妖精{}"
				}
			},
			fighting_sticker = {
				name = "属性",
				text = {
					"{X:fighting,C:white}格斗{}"
				}
			},
			fire_sticker = {
				name = "属性",
				text = {
					"{X:fire,C:white}火{}"
				}
			},
			grass_sticker = {
				name = "属性",
				text = {
					"{X:grass,C:white}草{}"
				}
			},
			hitem = {
				name = "物品牌",
				text = {
					"适用于{C:attention}#1#{}"
				}
			},
			holding = {
				name = "携带物品",
				text = {
					"这张牌附带",
					"一张{C:attention}#1#{}牌",
					"{C:inactive}（必须有空位）{}"
				}
			},
			lightning_sticker = {
				name = "属性",
				text = {
					"{X:lightning,C:white}电{}"
				}
			},
			metal_sticker = {
				name = "属性",
				text = {
					"{X:metal,C:white}钢{}"
				}
			},
			money_chance = {
				name = "给予金钱的几率",
				text = {
					"有{C:attention}#1#%{}的几率",
					"获得额外{C:money}$1{}"
				}
			},
			money_progress = {
				name = "获得的金钱进度",
				text = {
					"对获得的{C:money}金钱{}",
					"的{C:attention}#1#%{}进度增加{C:money}1${}"
				}
			},
			mult_progress = {
				name = "获得的倍数进度",
				text = {
					"对获得的{C:mult}倍数{}",
					"的{C:attention}#1#%{}进度增加{C:mult}+1{}"
				}
			},
			nature = {
				name = "多种性格",
				text = {
					"这张牌触发的{C:attention}卡牌点数{}",
					"会在这张牌生成后随机决定"
				}
			},
			p_poke_pokepack_jumbo_1 = {
				name = "特大宝可梦礼包",
				text = {
					"从{C:attention}#2#{}张{C:pink}能量{}或{C:item}物品{}牌中",
					"选择{C:attention}#1#{}张"
				}
			},
			p_poke_pokepack_jumbo_2 = {
				name = "特大宝可梦礼包",
				text = {
					"从{C:attention}#2#{}张{C:pink}能量{}或{C:item}物品{}牌中",
					"选择{C:attention}#1#{}张"
				}
			},
			p_poke_pokepack_mega_1 = {
				name = "超级宝可梦礼包",
				text = {
					"从{C:attention}#2#{}张{C:pink}能量{}或{C:item}物品{}牌中",
					"选择{C:attention}#1#{}张"
				}
			},
			p_poke_pokepack_mega_2 = {
				name = "超级宝可梦礼包",
				text = {
					"从{C:attention}#2#{}张{C:pink}能量{}或{C:item}物品{}牌中",
					"选择{C:attention}#1#{}张"
				}
			},
			p_poke_pokepack_normal_1 = {
				name = "宝可梦礼包",
				text = {
					"从{C:attention}#2#{}张{C:pink}能量{}或{C:item}物品{}牌中",
					"选择{C:attention}#1#{}张"
				}
			},
			p_poke_pokepack_normal_2 = {
				name = "宝可梦礼包",
				text = {
					"从{C:attention}#2#{}张{C:pink}能量{}或{C:item}物品{}牌中",
					"选择{C:attention}#1#{}张"
				}
			},
			p_poke_pokepack_normal_3 = {
				name = "宝可梦礼包",
				text = {
					"从{C:attention}#2#{}张{C:pink}能量{}或{C:item}物品{}牌中",
					"选择{C:attention}#1#{}张"
				}
			},
			p_poke_pokepack_normal_4 = {
				name = "宝可梦礼包",
				text = {
					"从{C:attention}#2#{}张{C:pink}能量{}或{C:item}物品{}牌中",
					"选择{C:attention}#1#{}张"
				}
			},
			percent_chance = {
				name = "百份比几率",
				text = {
					"这张牌的效果",
					"有{C:green}#1#%{}的几率触发",
					"{C:inactive}（几率不能提升）{}"
				}
			},
			playing_card_to_evolve = {
				name = "非自然进化",
				text = {
					"必需用到{C:attention}进化牌{}才能进化"
				}
			},
			poke_pink_seal_seal = {
				name = "粉红封蜡章",
				text = {
					"如果此牌在{C:attention}第一次出牌{}时计分",
					"产生一张{C:pink}能量{}牌"
				}
			},
			precise_energy_tooltip = {
				name = "精准的能量刻度",
				text = {
					"{s:0.8}使用{C:attention,s:0.8}小数{}{s:0.8}来显示所有的{C:pink,s:0.8}能量{}{s:0.8}加成的数值",
					"{C:attention,s:0.8}禁用{}{s:0.8}这个选项会对加成造成以下影响",
					"{C:attenion}1. {X:mult,C:white,s:0.8}乘成{}{s:0.8}倍数 - 使用小数",
					"{C:attenion}2. {s:0.8}加成{C:mult,s:0.8}倍数{}{s:0.8}及{C:chips,s:0.8}筹码{}{s:0.8} - 上取整至整数",
					"{C:attenion}3. {s:0.8}{C:mult,s:0.8}获得的倍数{}{s:0.8}及{C:chips,s:0.8}获得的筹码{}{s:0.8} - 小数单位会以",
					"{s:0.8}百分比显示，用于套用加成到基本{C:mult,s:0.8}倍数{}{s:0.8}及{C:chips,s:0.8}筹码{}",
					"{C:attenion}4. {C:money,s:0.8}金钱{}{s:0.8} - 小数单位会以百分比显示",
					"{s:0.8}用于有几率获得额外{C:money,s:0.8}金钱{}{s:0.8}的效果{}",
					"{C:inactive,s:0.8}（非宝可梦小丑牌总是使用精准刻度）{}"
				}
			},
			psychic_sticker = {
				name = "属性",
				text = {
					"{X:psychic,C:white}超能力{}"
				}
			},
			safaridesc = {
				name = "野生",
				text = {
					"只可以透过",
					"{C:attention}进化{}或特定{C:attention}的物品{}",
					"来获得此牌"
				}
			},
			stage1 = {
				name = "一次进化",
				text = {
					"已{C:attention}进化{}一次的",
					"宝可梦小丑牌"
				}
			},
			stage2 = {
				name = "二次进化",
				text = {
					"已{C:attention}进化{}两次的",
					"宝可梦小丑牌"
				}
			},
			undiscovered_energy = {
				name = "未发现",
				text = {
					"在无种子比赛中",
					"购买或使用此牌",
					"来了解功用"
				}
			},
			undiscovered_item = {
				name = "未发现",
				text = {
					"在无种子比赛中",
					"购买或使用此牌",
					"来了解功用"
				}
			},
			water_sticker = {
				name = "属性",
				text = {
					"{X:water,C:white}水{}"
				}
			}
		},
		Planet = {

		},
		Sleeve = {
			sleeve_poke_luminoussleeve = {
				name = "发亮牌套",
				text = {
					"所有小丑牌附有",
					"随机的{C:pink}属性{}贴纸"
				}
			},
			sleeve_poke_obituarysleeve = {
				name = "讣闻牌套",
				text = {
					"所有卡牌有{C:dark_edition}#1#{}"
				}
			},
			sleeve_poke_pokemonsleeve = {
				name = "训练家牌套",
				text = {
					"开局时获得",
					"{C:tarot,T:v_poke_goodrod}#1#{}优惠券及",
					"{C:tarot,T:c_poke_pokeball}#2#{}牌"
				}
			}
		},
		Spectral = {
			c_poke_masterball = {
				name = "大师球",
				text = {
					"产生一张随机",
					"{C:attention}传说的宝可梦小丑{}牌",
					"{C:inactive}（必须有空位）"
				}
			},
			c_poke_nightmare = {
				name = "梦魇",
				text = {
					"摧毁一张随机的宝可梦小丑牌",
					"并产生{C:attention}3{}张",
					"随机的{C:dark_edition}负片{}{C:pink}能量牌{}"
				}
			},
			c_poke_obituary = {
				name = "讣告",
				text = {
					"新增{C:pink}粉红{}封蜡章",
					"到选择的{C:attention}1{}张手牌中"
				}
			},
			c_poke_transformation = {
				name = "蜕变",
				text = {
					"把选择的或最左边的",
					"宝可梦进化到最高{C:attention}阶段{}",
					"并给予{}+1{} {C:pink}能量{}"
				}
			},
			c_poke_ultraball = {
				name = "高级球",
				text = {
					"产生一张随机",
					"{C:attention}二次进化的宝可梦小丑{}牌",
					"{C:inactive}（必须有空位）"
				}
			}
		},
		Tag = {
			tag_poke_pocket_tag = {
				name = "宝可梦标签",
				text = {
					"获得一个免费的",
					"{C:pink}超级宝可梦礼包"
				}
			},
			tag_poke_safari_tag = {
				name = "野生标签",
				text = {
					"下个在商店中",
					"有免费的{C:safari}野生{}宝可梦"
				}
			},
			tag_poke_shiny_tag = {
				name = "发光标签",
				text = {
					"下个在商店中的小丑牌",
					"是免费并带有{C:colorless}发光{}"
				}
			},
			tag_poke_stage_one_tag = {
				name = "一次进化标签",
				text = {
					"下个在商店中",
					"有免费的{C:attention}一次进化{}宝可梦"
				}
			}
		},
		Tarot = {

		},
		Voucher = {
			v_poke_energyresearch = {
				name = "能量研究",
				text = {
					"{C:pink}+3{}能量上限"
				}
			},
			v_poke_energysearch = {
				name = "能量分析",
				text = {
					"{C:pink}+2{}能量上限"
				}
			},
			v_poke_goodrod = {
				name = "好钓竿",
				text = {
					"{C:attention}宝可梦{}礼包",
					"有额外{C:attention}一{}张牌在里面"
				}
			},
			v_poke_superrod = {
				name = "厉害钓竿",
				text = {
					"{C:attention}宝可梦{}礼包",
					"有额外{C:attention}一{}张牌在里面"
				}
			}
		}
	},
	misc = {
		achievement_descriptions = {

		},
		achievement_names = {

		},
		challenge_names = {
			c_poke_nuzlocke = "Nuzlocke",
			c_poke_poke_master = "宝可梦大师"
		},
		dictionary = {
			b_energy_cards = "能量牌",
			b_item_cards = "物品牌",
			b_save = "SAVE",
			cubone_marowak_infoqueue = "卡拉卡拉及嘎啦嘎啦",
			farfetchd_infoqueue = "大葱鸭",
			goodrod_variable = "好钓竿",
			k_energy = "能量",
			k_item = "物品",
			k_poke_pocket_pack = "宝可梦礼包",
			k_poke_safari = "野生",
			pinkseal_variable = "粉红封蜡章",
			poke_credits_actualcredits = "制作员名单",
			poke_credits_community_manager = "社群经理：",
			poke_credits_designer = "设计辅助：",
			poke_credits_developer = "开发者：",
			poke_credits_graphics = "图形设计：",
			poke_credits_lead = "主导开发：",
			poke_credits_localization = "翻译员：",
			poke_credits_promotion_producer = "推广员：",
			poke_credits_quality_assurance_main = "品质保证：",
			poke_credits_thanks = "鸣谢于",
			poke_destroyed_ex = "摧毁！",
			poke_disguise_broken = "画皮已脱落！死亡没有防止！",
			poke_disguise_intact = "画皮被触发！防止了死亡！",
			poke_dragon_ex = "龙化！",
			poke_energized_ex = "能量全满！",
			poke_evolve_level = "升级！",
			poke_evolve_success = "进化！",
			poke_explosion_ex = "自爆！",
			poke_faint_ex = "濒死！",
			poke_fire_blast_ex = "大字爆炎！",
			poke_gift_ex = "礼物！",
			poke_lick = "舔舔",
			poke_lick_ex = "舔舔！",
			poke_metal_ex = "钢化！",
			poke_mime_ex = "模仿！",
			poke_nido_ex = "Nido!",
			poke_petal_dance = "花瓣舞",
			poke_petal_dance_ex = "花瓣舞！",
			poke_pinsir_pin = "钉选！",
			poke_pinsir_remove_pin = "解钉！",
			poke_plus_energy = "+1 能量",
			poke_plus_pokeitem = "+1 物品",
			poke_screech_ex = "尖叫！",
			poke_settings_header_norequired = "无需重启：",
			poke_settings_header_required = "需要重启：",
			poke_settings_jokers_only = "只用非宝可梦小丑牌？",
			poke_settings_no_evolutions = "禁止进化？",
			poke_settings_pokeballs = "允许精灵球？",
			poke_settings_pokedex_number = "显示图鉴号码？",
			poke_settings_pokemon_gen_one = "只用第一世代的宝可梦？",
			poke_settings_pokemon_only = "只使用宝可梦？",
			poke_settings_pokemon_precise_energy = "使用精准的能量刻度？",
			poke_settings_pokemon_splash = "片头中显示随机的宝可梦牌？",
			poke_settings_shiny_playing_cards = "扑克牌可有发光版本？",
			poke_settings_unlimited_energy = "能量无上限？",
			poke_solar = "日光束",
			poke_solar_ex = "日光束！",
			poke_surf_ex = "冲浪！",
			poke_tangela_bonus = "全中！",
			poke_tera_ex = "变更！",
			poke_thunder_ex = "打雷！",
			poke_twister_ex = "龙卷风！",
			poke_unlimited_energy = "无上限",
			poke_x_scissor = "十字剪",
			poke_x_scissor_ex = "十字剪！",
			pokeball_variable = "精灵球",
			snorlax_infoqueue = "卡比兽"
		},
		labels = {
			colorless_sticker = "属性贴纸",
			dark_sticker = "属性贴纸",
			dragon_sticker = "属性贴纸",
			earth_sticker = "属性贴纸",
			fairy_sticker = "属性贴纸",
			fighting_sticker = "属性贴纸",
			fire_sticker = "属性贴纸",
			grass_sticker = "属性贴纸",
			k_poke_safari = "野生",
			lightning_sticker = "属性贴纸",
			metal_sticker = "属性贴纸",
			poke_pink_seal_seal = "粉红封蜡章",
			poke_shiny = "发光",
			psychic_sticker = "属性贴纸",
			water_sticker = "属性贴纸"
		},
		v_dictionary = {

		},
		v_text = {
			ch_c_poke_add_joker_slots = {
				"打败Boss盲注后，{C:attention}+1{}小丑牌字段{C:inactive}（最多5个）"
			},
			ch_c_poke_nuzlocke = {
				"每个赌注的第一个商店必定有{C:attention}丑角礼包"
			}
		}
	}
}
