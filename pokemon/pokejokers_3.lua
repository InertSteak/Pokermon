...
local farfetchd={
  ...
  loc_txt = {
    ...
    text = {
      ...
      "{C:inactive}(2 would give {X:red,C:white}X#4#{}{C:inactive} total, etc){}"
    }
  },
  loc_vars = function(self, info_queue, center)
    type_tooltip(self, info_queue, center)
    info_queue[#info_queue+1] = G.P_CENTERS.c_poke_leek
    return {vars = {center.ability.extra.Xmult, ''..(G.GAME and G.GAME.probabilities.normal or 1), center.ability.extra.odds, center.ability.extra.Xmult*2}}
  end,
  rarity = 2,
  cost = 6,
  stage = "Basic",
