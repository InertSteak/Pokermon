...
poke_get_upgraded_pack_center = function(center, into, from)
  if not from then
...
function get_pack(_key, _type)
  ...
  if _key == 'shop_pack' then
    if next(SMODS.find_card('j_poke_mega_drampa')) then
      center = poke_get_upgraded_pack_center(center, 'mega') or center
    elseif next(SMODS.find_card('j_poke_drampa')) then
      center = poke_get_upgraded_pack_center(center, 'jumbo') or center
    end
  end
  return center
end

poke_upgrade_shop_pack = function(pack, into)
  local old_center = pack.config.center
  local new_center = poke_get_upgraded_pack_center(old_center, into)
...
poke_upgrade_all_shop_packs = function(into)
  if G.shop_booster and G.shop_booster.cards then
    for _, pack in ipairs(G.shop_booster.cards) do
      poke_upgrade_shop_pack(pack, into)
    end
  end
end
