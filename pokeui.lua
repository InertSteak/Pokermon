    ...
    if G.jokers and G.jokers.highlighted and G.jokers.highlighted[1] then
      ...
      if selected.config.center.stage then
        G.FUNCS.overlay_menu{
          definition = create_UIBox_pokedex_jokers(get_family_keys(selected.config.center.key)),
        }
      end
    end
  end
...
function Controller:queue_R_cursor_press(x, y)
    ...
    if clicked and type(clicked) == 'table' and clicked.config and type(clicked.config) == 'table' and clicked.config.center then
      if clicked.config.center.stage then
        ...
        G.FUNCS.overlay_menu{
          definition = create_UIBox_pokedex_jokers(get_family_keys(clicked.config.center.key), menu),
        }
      end
    end
end